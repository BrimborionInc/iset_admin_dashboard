...
function usage() {
  return [
    'Usage: node scripts/apply-20260825-typed-lineage.js --target-env <dev|test|prod> --env-file <path> --yes [options]',
    '',
    'This executor is checksum-pinned to one migration.',
    'It does not pass the SQL file to the general runner: it proves the file hash and exact',
    'nine-literal DDL set, applies those literals, then records the canonical checksum.',
    'Local DEV has no AWS dependency. For TEST and PROD, the outer control plane records its',
    'explicit STS/SSM proof; this remote executor independently verifies the EC2 instance',
    'identity document without AWS credentials and binds it to that exact recorded context.',
    '',
    'Options:',
    ...
    '  --compact-output                     Emit a bounded result summary instead of full evidence.',
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (['--target-env', '--env-file', '--expected-aws-account-id', '--expected-ssm-instance-id', '--run-token', '--evidence-out'].includes(token)) {
...
function readDbConfig(envFile) {
  ...
  if (!config.host || !config.user || !config.password || !config.database || !Number.isInteger(config.port)) {
...
function defaultEvidencePath(targetEnv) {
  ...
  return path.join(REPO_ROOT, 'tmp', 'migration-evidence', `${targetEnv}-${MIGRATION_FILENAME}-${timestamp}.json`);
}

function validateArgs(args) {
  if (!args.targetEnv || !args.envFile) throw new Error(usage());
  if (!ENVIRONMENT_CONTRACTS[args.targetEnv]) throw new Error('--target-env must be dev, test, or prod');
  ...
  if (args.targetEnv === 'test' || args.targetEnv === 'prod') {
    ...
    if (!String(args.runToken || '').trim()) throw new Error(`${label} requires a nonempty --run-token`);
  }
  if (args.targetEnv === 'dev' && (args.expectedAwsAccountId || args.expectedSsmInstanceId || args.runToken)) {
...
function summarizeEvidence(evidence, output = {}) {
  const operationStates = evidence?.finalProof?.operationStates || {};
  return {
    ...
    executor: evidence?.executor || '20260825-typed-lineage-bounded',
    ...
    finalOperationStates: operationStates,
    seriesStructureStable: Boolean(
      evidence?.preflight &&
      evidence?.finalProof &&
      evidence.preflight.tables?.cfa_series?.structureHash === evidence.finalProof.tables?.cfa_series?.structureHash &&
      evidence.preflight.tables?.funding_overview_series?.structureHash === evidence.finalProof.tables?.funding_overview_series?.structureHash
    ),
...
function persistEvidenceWithFallback({
  ...
  try {
    ...
    const fallbackPath = path.join(
      fallbackDirectory,
      `typed-lineage-${String(evidence?.decision || 'UNKNOWN').toLowerCase()}-${process.pid}-${Date.now()}.json`
    );
    return {
...
async function main() {
  ...
  const isRemote = args.targetEnv === 'test' || args.targetEnv === 'prod';
  const instanceIdentityProvider = isRemote ? createEc2InstanceIdentityProvider() : null;
  const evidencePath = args.evidenceOut || defaultEvidencePath(args.targetEnv);
  let evidence;
  let executionError = null;
  try {
    evidence = await executeTypedLineageMigration({
    ...
    evidence = error.evidence || {
      ...
      targetEnv: args.targetEnv,
      decision: 'FAILED',
      failure: { code: error.code || 'typed_lineage_cli_failed', message: error.message },
    };
  }
  ...
  const output = JSON.stringify(args.compactOutput
    ? summarizeEvidence(persisted.evidence, fullOutput)
    : fullOutput, null, 2);
  if (executionError || persisted.usedFallback) {
    // A persistence failure after COMPLETE is an evidence-output failure, not a schema failure.
...
if (require.main === module) {
  main().catch((error) => {
    const fallback = path.join(os.tmpdir(), `typed-lineage-cli-failure-${process.pid}.json`);

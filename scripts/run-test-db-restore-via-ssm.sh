...
REMOTE_COMMANDS=(
  ...
  "MYSQL_PWD=\"\$DB_PASS\" mysql --protocol TCP --ssl-mode=REQUIRED -h $(shell_quote "$DB_HOST") -P $(shell_quote "$DB_PORT") -u \"\$DB_USER\" -e \"DROP DATABASE IF EXISTS \\\`$DB_NAME\\\`; CREATE DATABASE \\\`$DB_NAME\\\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;\""
  "if [[ $(shell_quote "$REMOTE_SNAPSHOT") == *.gz ]]; then gzip -dc $(shell_quote "$REMOTE_SNAPSHOT") | sed -E 's/DEFINER=\`[^\\`]+\`@\`[^\\`]+\`//g' | MYSQL_PWD=\"\$DB_PASS\" mysql --protocol TCP --ssl-mode=REQUIRED -h $(shell_quote "$DB_HOST") -P $(shell_quote "$DB_PORT") -u \"\$DB_USER\" $(shell_quote "$DB_NAME"); else sed -E 's/DEFINER=\`[^\\`]+\`@\`[^\\`]+\`//g' $(shell_quote "$REMOTE_SNAPSHOT") | MYSQL_PWD=\"\$DB_PASS\" mysql --protocol TCP --ssl-mode=REQUIRED -h $(shell_quote "$DB_HOST") -P $(shell_quote "$DB_PORT") -u \"\$DB_USER\" $(shell_quote "$DB_NAME"); fi"

...
const packPath = join(qualificationRoot, 'packs', 'admin-lint.pack.json');
...
const sourceScope = collectAdminLintSourceScope(repositoryRoot);
const dependencyScope = collectAdminLintDependencyScope(repositoryRoot);
...
const localEnvironmentIdentity = createEnvironmentIdentity({
  manifestRefs: [reference('environment-class', 'phase3d.local-read-only', digestCanonical({ class: 'local' }))],
...
const testPackVersions = createTestPackVersions([{
  ...
  material: {
    ...
    configDigest: bundle.pack.inputs.find((input) => input.role === 'native-config').contentDigest.value,
...
const directHarnessVersion = createHarnessVersion({
  manifestRefs: [packManifestRef],
  material: {
    qualificationSourceDigest: digestCanonical({ mode: 'direct-native-eslint' }),
...
function attempt(sequence, lane) {
  return createAttemptId(`00000000-0000-4000-8000-${String(sequence).padStart(12, lane === 'direct' ? '7' : '8')}`);
}
...
function sourceState() {
  return {
    ...
    lock: digestBytes(readFileSync(packageLockPath)),
    config: digestBytes(readFileSync(join(repositoryRoot, '.eslintrc.cjs'))),
    binary: digestBytes(readFileSync(join(repositoryRoot, 'node_modules/eslint/bin/eslint.js'))),
    sourceScope: collectAdminLintSourceScope(repositoryRoot).contentDigest,
    dependencyScope: collectAdminLintDependencyScope(repositoryRoot).contentDigest,
    cache: fileState(eslintCachePath),
  };
}

test('pack, exact cumulative registry, role boundary, and src-only authority validate', () => {
  assert.equal(bundle.pack.packId, 'admin-lint');
  assert.equal(bundle.pack.packVersion, '1.0.2');
  assert.equal(bundle.pack.requiredAdapter.adapterVersion, '2.0.0');
  assert.equal(
    bundle.pack.inputs.find((input) => input.role === 'product-source-scope').contentDigest.value,
  ...
  assert.deepEqual(
    bundle.roleManifest.packExternalReadOnlyInputs['admin-lint'],
  ...
  assert.ok(bundle.pack.limitations.includes('Server, scripts, tests, and files outside src are not covered by this pack.'));
});

test('source and resolved dependency scopes are exact, deterministic, and mutation-sensitive', () => {
  assert.equal(sourceScope.pattern, ADMIN_LINT_SOURCE_SCOPE);
  ...
  assert.equal(collectAdminLintSourceScope(repositoryRoot).contentDigest, sourceScope.contentDigest);
  assert.equal(dependencyScope.scope, ADMIN_LINT_DEPENDENCY_SCOPE);
  assert.deepEqual(dependencyScope.packages.map((entry) => entry.packageName), ADMIN_LINT_DEPENDENCY_PACKAGES);
  assert.equal(collectAdminLintDependencyScope(repositoryRoot).contentDigest, dependencyScope.contentDigest);
...
test('validation rejects promotion, writes, cache/fix commands, and stale config/lock/binary/scopes', () => {
  ...
  write.declaredEffects.writePaths.push('src');
  ...
  for (const role of ['dependency-lock', 'native-config', 'native-runner', 'native-dependency-scope', 'product-source-scope']) {
    const stale = structuredClone(bundle.pack);
    stale.inputs.find((input) => input.role === role).contentDigest.value = '0'.repeat(64);
    expectPackCode(
      () => verifyPackInputs(stale, repositoryRoot, bundle.roleManifest.packExternalReadOnlyInputs['admin-lint']),
      'INPUT_FINGERPRINT_DRIFT',
    );
  }

  for (const substitutedScope of ['src/**/*.js', 'src/**/*.{js,jsx,ts}']) {
    const changed = structuredClone(bundle.pack);
    const input = changed.inputs.find((entry) => entry.role === 'product-source-scope');
    changed.declaredEffects.readPaths[changed.declaredEffects.readPaths.indexOf(input.path)] = substitutedScope;
    input.path = substitutedScope;
    expectPackCode(
      () => verifyPackInputs(changed, repositoryRoot, bundle.roleManifest.packExternalReadOnlyInputs['admin-lint']),
      'UNDECLARED_EXTERNAL_INPUT',
    );
  }
});

test('registry, role, and native package alias conflicts fail closed', () => {
  ...
  role.packExternalReadOnlyInputs['admin-lint'].push('scripts/**/*.js');
...
test('ten frozen-identity advisory known-good attempts pass with no source or cache residue', async () => {
...
test('five additional direct and advisory known-good attempts match exactly', async () => {
...
test('deliberate native lint error fails identically in direct and advisory paths', async () => {
  const before = sourceState();
  ...
  assert.match(advisoryOutput, /notDeclaredForLintCertification.*no-undef/u);
  assert.match(direct.stdout.toString('utf8'), /notDeclaredForLintCertification.*no-undef/u);
  assert.equal(compareAdvisoryToDirect(direct.record, advisory).status, 'matched');
  assert.deepEqual(sourceState(), before);
});

test('malformed evidence, substituted command, and result disagreement fail closed', async () => {

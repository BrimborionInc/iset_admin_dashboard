...
- Editing is blocked if the case status is already Approved or Denied, except for reporting-only denied applications that remain editable for ILMP corrections. Otherwise, coordinators may update answers on the applicant's behalf when they have source evidence.
...
const buildSystemPrompt = ({ focusTitle, aiContext }) => {
  ...
  if (isCoordinatorPathWorkflowContext({ focusTitle, aiContext: safeContext })) {
    sections.push(
      ...
      '- When the question is about historical client-file records, explain the Case Header quick actions `Add existing action plan`, `Add existing intervention`, and `Upload existing documents` as tools for adding records that already existed before PATH.',
      ...
      '- For historical supporting documents, keep the answer practical: use Upload existing documents and attach the file to the correct client, case, or action plan.',

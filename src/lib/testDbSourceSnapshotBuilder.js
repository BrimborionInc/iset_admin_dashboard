...
const FILTERED_DATA_TABLES = [
  ...
  },
  {
    table: 'iset_runtime_config',
    where: [
      "(scope = 'assessment' AND k = 'coordinator.costing.line_item_defaults')",
      "(scope = 'finance' AND k IN ('payment.intervention.payment_type_map', 'payment.evidence.rules', 'payment.payee_type_options', 'email.routing'))",
    ].join(' OR '),
    description: 'Finance/payment runtime configuration rows',
  },
];

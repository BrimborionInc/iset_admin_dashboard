...
const SEEDED_GUIDANCE_ENTRIES = [
  ...
  },
  {
    ...
    restrictionsText:
      "This is an annual approved-funding report for CRF/EI intervention advances, not a Sage ledger, full payment-history ledger, or actual-paid accounting report. PATH payment status and recorded paid/confirmed values are operations-side follow-up records beside approved funding. Carry-over is a best-effort estimate for cross-fiscal review.",
    answerStyleText:
      "Explain the page as a finance review job aid. Lead with what the user can do on the page, then clarify report basis and payment-status caveats when relevant.",
    guidanceText:
      "`Financial Reports` is the annual `ISET Advances and Active Clients` report. It reviews approved CRF/EI intervention funding, active client counts, regional totals, and intervention-level detail for the selected fiscal year and participant home region. The Intervention detail view defaults to `Funded interventions only`, so zero-dollar approved interventions such as Employment counselling or Career research & exploration stay out of the advances view unless staff choose `All approved interventions`. The detail table includes the funding source, approved funding amount, category allocations, and PATH payment follow-up state. Use `Export to Excel` when finance needs the workbook-style package from the visible filtered rows. Do not treat the page as Sage/AP authority or as a full actual-paid ledger.",
  },
  {
...
const SEEDED_GUIDANCE_EXAMPLES = [
  ...
  },
  {
    ...
    roleContext: ["NWAC Administrator", "System Administrator", "Finance Manager"],
    mustMention: [
      "ISET Advances and Active Clients",
      "annual approved-funding report",
      "CRF/EI",

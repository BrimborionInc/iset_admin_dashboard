...
const SEEDED_GUIDANCE_ENTRIES = [
  ...
  },
  {
    ...
    stepsText:
      "On the Home Dashboard, select Pending Completion in the Work Queue. In Work Queue Items, open the item name. Approved application rows open Application Workspace with an explicit post-decision step: Approval letters first, then Funding forms and signatures after the approval letter has been sent. Denied application rows open the denial-letter step until that letter is sent. Approved new intervention proposal and approved intervention revision rows open Case Workspace on the intervention approval-letter follow-up.",
    sideEffectsText:
      "Pending Completion does not itself complete, close, archive, assign, or decide the file. It is a launch queue for outstanding post-decision work. Approved application rows remain until the application workflow is completed or closed. Denied application rows remain only until the denial letter is sent; sending that letter completes the application. Approved intervention proposal/revision rows remain until the intervention-scoped client approval/funding revision letter is sent.",
    restrictionsText:
      "Do not describe Pending Completion as a queue of already completed or archived records. It intentionally includes decision-recorded approved application files that still need follow-through and denied application files whose denial letter has not yet been sent. It does not include ordinary approved/planned interventions, historical/backloaded interventions, or auto-assessment interventions created by an application approval.",
    answerStyleText:
    ...
    guidanceText:
      "`Pending Completion` is the post-decision follow-through queue. It is for work where the decision has been recorded but PATH still needs letters, funding-form/signature work, final checklist/closeout, or intervention approval-letter follow-up before the file is truly complete. Approved application rows stay there until the application is completed or closed and route first to `Approval letters`, then to `Funding forms and signatures` after the approval letter is sent. Denied application rows stay there only until the denial letter is sent, because sending the denial letter completes the application. Approved new intervention proposals and approved intervention revisions also appear there until the intervention-scoped client approval or funding revision letter is sent. Staff normally open the row through the item name and complete the work in Application Workspace or Case Workspace.",
  },
  {
  ...
  {
    slug: "finance-reports-iset-advances-active-clients",
    title: "ISET Advances and Active Clients report",
    surface: HELP_PANEL_SURFACE,
    priority: 119,
    sourceType: "workflow",
    coverageDomain: "Finance / Reporting",
    coverageStatus: "verified",
    lastReviewedAt: "2026-05-26",
    routePatterns: ["/finance/reports"],
    helpTitles: [
      "Financial Reports",
    ...
    keywords: [
      ...
      "ISET Advances and Active Clients",
      "funded clients",
      "annual advances report",
      "CRF",
      "EI",
      "funded interventions",
      "zero dollar interventions",
      "carry-over",
      "payment status",
      "payment follow-up",
      "Excel export",
    ...
    stateHints: ["finance-reports", "annual-advances-report"],
    workflowStates: ["approved", "annual_report", "funded_interventions"],
    sourceRefs: [
      "src/pages/finance/FinanceReportsPage.jsx",
      "src/helpPanelContents/financeReportsHelp.js",
      "src/pages/finance/financeInterventionReportExport.js",
      "docs/dashboards/financial-reports-dashboard.md",
      "isetadminserver.js",
    ],
    expectedAnchors: [
      "annual approved-funding report",
      "CRF and EI",
      "funded clients",
      "approval date",
    ...
    restrictionsText:
      "This is an annual approved-funding report for funded CRF/EI intervention advances by approval date, not a Sage ledger, full payment-history ledger, or actual-paid accounting report. PATH payment status and recorded paid/confirmed values are operations-side follow-up records beside approved funding. Carry-over is a best-effort estimate for cross-fiscal review. Describe the page as a PATH-side staff review and export view, with any external finance/audit context framed as handoff or reconciliation outside PATH.",
    answerStyleText:
  ...
  },
  {
    ...
    title: "Reconciling Financial Reports with Data and Results",
    surface: HELP_PANEL_SURFACE,
    priority: 118,
    sourceType: "workflow",
    coverageDomain: "Finance / Reporting",
    coverageStatus: "verified",
    lastReviewedAt: "2026-05-26",
    routePatterns: ["/finance/reports", "/reporting/data-and-results"],
    helpTitles: [
      "Financial Reports",
      "ISET Advances and Active Clients",
      "Data and Results",
    ...
    stateHints: ["finance-reports", "data-and-results", "report-reconciliation"],
    workflowStates: ["approved", "funded_interventions", "reporting"],
    sourceRefs: [
      "src/pages/finance/FinanceReportsPage.jsx",
      "src/helpPanelContents/financeReportsHelp.js",
    ...
    doNotSay: [
      "should match",
      "same report",
      "Data and Results is the finance advances report",
      "Financial Reports uses completed date",
    ],
    applicabilityText:
      "Use when staff compare Financial Reports with Reporting > Data and Results, ask why client/intervention counts or dollars differ, or ask which page to use for funded advances.",
    stepsText:
      "For funded advances, use Budgets and Finance > Financial Reports. For operational reporting activity, use Reporting > Data and Results and check that section's Show, Status, Results view, and Date controls before comparing numbers.",
    sideEffectsText:
      "Changing either dashboard's filters changes only the visible report scope. It does not mutate case, intervention, finance, or reporting-package data.",
    restrictionsText:
      "Do not promise the dashboards will reconcile without aligning the basis. Financial Reports is funded approved CRF/EI advances by intervention approval date. Data and Results Interventions is controlled by intervention status and start date, end date, or payment month.",
    answerStyleText:
      "Lead with the basis difference, then tell staff which controls to check. Keep the answer calm and reconciliation-focused.",
    guidanceText:
      "Financial Reports and Data and Results are not expected to match by default. `Financial Reports` is the approved-funding advances view: funded CRF/EI interventions approved in the selected fiscal year, with funded clients deduplicated from the visible funded rows. `Data and Results` is the operational reporting view: its Interventions section depends on the selected Show value, intervention Status, Results view, and start date, end date, or payment month basis. Use Financial Reports for finance advances and workbook reconciliation; use Data and Results for reporting activity, completed/planned/active/cancelled intervention counts, payment-month cost allocation, client results, and action-plan status reporting.",
  },
  {
  ...
  {
    slug: "esdc-ilmp-participant-batch-export-flow",
    title: "ILMP participant batch XML export flow",
    surface: HELP_PANEL_SURFACE,
    priority: 127,
    sourceType: "workflow",
    ...
    routePatterns: ["/esdc/participants"],
    helpTitles: [
    ...
    stateHints: ["esdc-ilmp", "participant-export"],
    workflowStates: ["ready", "needs_review", "blocked", "exported", "manual_upload"],
    sourceRefs: [
      "src/pages/esdc/EsdcParticipantSubmissionsPage.jsx",
    ...
    expectedAnchors: [
      ...
      "Generate batch XML",
      "ready clients",
      "Save XML and mark exported",
      "Download XML and mark exported",
      "manual upload outside PATH",
    ],
    doNotSay: [
      "PATH submits to ESDC",
      "uploaded to ESDC automatically",
      "submitted by PATH",
      "marks submitted",
      "submission complete at ESDC",
      "ESDC portal upload is automatic",
    ],
    applicabilityText:
      "Use when staff ask how to create, save, download, export, or upload the participant ILMP XML file from the ILMP Submissions & Exports dashboard.",
    stepsText:
      "Open ILMP Submissions & Exports. In Participant submission queue, run Validate all so readiness reflects current case data. Open linked case workspaces for blocked records or warnings that need review. When the ready clients should go into the next file, choose Generate batch XML. Review the ready/excluded counts and filename, then choose Save XML and mark exported or Download XML and mark exported depending on browser support. Upload the downloaded file manually through the external ESDC process.",
    sideEffectsText:
      "Saving or downloading records the included clients as exported in PATH, stores an export-history entry with the generated XML, and refreshes the queue/history widgets. Non-ready records are excluded from that file and remain in the queue for correction.",
    restrictionsText:
      "PATH does not directly submit or upload participant XML to ESDC from this dashboard. Avoid staff-facing submitted/submission language for this PATH step; use exported/downloaded/manual upload unless talking about external ESDC records or schema terminology.",
    answerStyleText:
      "Give the operational sequence first. Keep language task-focused and explicit that ESDC upload is manual outside PATH.",
    guidanceText:
      "To prepare a participant ILMP file, open `ILMP Submissions & Exports` and work from the `Participant submission queue`. Run `Validate all`, fix blockers from the linked case workspace, then choose `Generate batch XML` when the ready clients should be exported. The modal shows ready and excluded counts plus the filename. Use `Save XML and mark exported` when the browser supports the native save dialog, or `Download XML and mark exported` in the normal download flow. PATH records those clients as exported and stores the generated XML in history; it does not upload the file to ESDC. Staff still complete the external manual ESDC upload outside PATH.",
  },
  {
    slug: "esdc-ilmp-export-history-xml-snapshot",
    title: "Recent ILMP exports history and XML snapshot",
    surface: HELP_PANEL_SURFACE,
    priority: 125,
    sourceType: "workflow",
    coverageDomain: "ESDC / ILMP",
    ...
    routePatterns: ["/esdc/participants"],
    helpTitles: [
      "Participant submissions history",
      "Recent ILMP exports",
      "ILMP Submissions & Exports",
      "Help and Tutorials",
    ],
    ...
    stateHints: ["esdc-ilmp", "export-history"],
    workflowStates: ["exported", "history", "requeue", "manual_upload"],
    sourceRefs: [
    ...
    expectedAnchors: [
      ...
      "not regenerated from current client data",
      "Requeue",
    ],
    doNotSay: [
      "live XML",
      "regenerated from current client data",
      "updates automatically when the client changes",
      "submits to ESDC",
      "upload status",
      "checksum is the primary staff task",
    ],
    applicabilityText:
      "Use when staff ask what Recent ILMP exports shows, whether the XML tab is live or historical, who downloaded a file, what requeue does, or how to compare/export prior files.",
    stepsText:
      "Add Recent ILMP exports from the dashboard palette when prior export details are needed. Select a file in the history table. Use Summary for recorded file path/name, downloaded time, downloaded-by display name, and client count. Use Clients exported to review the clients and open their case workspaces. Use XML to preview/copy the exact XML stored with that export. Use Requeue only when those clients need to return to the queue for a replacement export.",
    sideEffectsText:
      "Requeue returns every client in the selected export to the ILMP queue so a replacement XML file can be generated. It does not contact ESDC and it does not change any external upload status.",
    restrictionsText:
      "The XML tab is an audit snapshot from the export history record, not live XML based on current client data. If staff need corrected current data, requeue and generate a new export after fixing the case data.",
    answerStyleText:
      "Answer snapshot/live-data questions directly. Keep the history widget framed around audit, comparison, client follow-up, XML copy, and replacement export tasks.",
    guidanceText:
      "`Recent ILMP exports` is the optional history widget for downloaded participant XML files. Select an export to use `Summary`, `Clients exported`, and `XML`. Summary shows the recorded file path/name, downloaded time, downloaded-by display name, and number of clients exported. `Clients exported` lists the clients in that file and links them back to case workspaces. `XML` shows the stored XML snapshot generated at the time of export; it is not live XML regenerated from current client data. Use `Requeue` only when the clients from that export need to go back to the queue for a replacement XML file. Requeue does not contact ESDC.",
  },
  {
...
const SEEDED_GUIDANCE_EXAMPLES = [
  ...
  },
  {
    ...
    roleContext: ["ISET Coordinator", "NWAC Administrator", "System Administrator"],
    mustMention: [
      "No",
      "approval authorizes funding",
      "authorization ceiling",
  ...
  {
    guidanceSlug: "finance-reports-iset-advances-active-clients",
    sortOrder: 85,
    coverageStatus: "verified",
    evalFixtureId: "finance-reports-annual-advances-purpose",
    routeContext: ["/finance/reports"],
    roleContext: ["NWAC Administrator", "System Administrator"],
    mustMention: [
      "ISET Advances and Active Clients",
      "annual approved-funding report",
      "CRF/EI",
  ...
  },
  {
    ...
    sortOrder: 86,
    coverageStatus: "verified",
    evalFixtureId: "finance-reports-data-results-comparison",
    routeContext: ["/finance/reports", "/reporting/data-and-results"],
    roleContext: ["NWAC Administrator", "System Administrator"],
    mustMention: [
      "Financial Reports",
      "Data and Results",
    ...
    questionText: "Why does Financial Reports not match Data and Results?",
    answerText:
      "`Financial Reports` and `Data and Results` answer different reporting questions. Financial Reports is the approved-funding advances view for funded CRF/EI interventions approved in the selected fiscal year. Data and Results is controlled by reporting choices such as intervention status, start date, end date, and payment month. Use Financial Reports for finance advances; use Data and Results for reporting activity and outcomes.",
  },
  {
  ...
  {
    guidanceSlug: "esdc-ilmp-participant-batch-export-flow",
    sortOrder: 115,
    coverageStatus: "verified",
    evalFixtureId: "ilmp-participant-batch-export-flow",
    routeContext: ["/esdc/participants"],
    roleContext: ["NWAC Administrator", "System Administrator"],
    mustMention: [
      "ILMP Submissions & Exports",
      "Participant submission queue",
    ...
    questionText: "How do I export the ILMP participant XML file for upload?",
    answerText:
      "Open `ILMP Submissions & Exports` and use the `Participant submission queue`. Run `Validate all`, fix any blockers from the linked case workspace, then choose `Generate batch XML` when the ready clients should be included. In the modal, review the ready/excluded counts and filename, then use `Save XML and mark exported` or `Download XML and mark exported`. PATH records the clients as exported and stores the generated XML; staff still upload the downloaded XML manually outside PATH through the external ESDC process.",
  },
  {
    guidanceSlug: "esdc-ilmp-export-history-xml-snapshot",
    sortOrder: 116,
    coverageStatus: "verified",
    evalFixtureId: "ilmp-export-history-xml-snapshot",
    routeContext: ["/esdc/participants"],
    roleContext: ["NWAC Administrator", "System Administrator"],
    mustMention: [
      "Recent ILMP exports",
    ...
    questionText: "Does the XML tab show live XML or the XML from the export?",
    answerText:
      "The `XML` tab in `Recent ILMP exports` shows the stored XML snapshot from the time that export was downloaded. It is not regenerated from current client data. Use `Summary` for the recorded file path/name, downloaded time, downloader display name, and client count, and `Clients exported` to open the exported clients. If current case data needs to be reflected, fix the case data, use `Requeue` to return those clients to the ILMP queue, and generate a replacement XML file.",
  },
  {

...
const ApplicationsWidgetHelp = () => (
  <div>
    ...
    <ul>
      <li><strong>Status:</strong> Check whether the file is a draft, returned to its submitter, with the Regional Manager, with the Decision Maker, or in post-decision completion.</li>
    ...
    <ul>
      ...
      <li>EI status controls CRF/EI funding alignment, not whether the application is in Regional Manager or Decision Maker review.</li>
      <li>A funded approval may require the exact application&apos;s CFA and funding forms. A zero-funding approval does not.</li>
...
- Explain that review ownership comes from the two-step stage: Regional Manager Pending Review before Decision Maker Pending Decision, with Decision Maker changes returning through the RM to the recorded submitter.
- Explain that EI status controls CRF/EI funding alignment, not review queue ownership.
- Explain that funded approvals may require the exact application-linked CFA and signatures during post-decision follow-up; zero-funding approvals do not.

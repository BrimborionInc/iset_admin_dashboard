...
const buildCaseFromWorkspaceApi = (caseId, payload) => {
  ...
  const actionPlans = sortActionPlansByRecency(rawActionPlans.map(plan => {
    ...
    return {
      ...
      budgetPot: plan.budgetPotId || plan.budget_pot_id || plan.budgetPot || plan.budget_pot || null,
      budgetPotId: plan.budgetPotId || plan.budget_pot_id || null,
      budgetPotCode: plan.budgetPotCode || plan.budget_pot_code || null,
      budgetPotName: plan.budgetPotName || plan.budget_pot_name || null,
      budgetPotLabel: plan.budgetPotLabel || plan.budgetPotDisplayLabel || plan.budget_pot_label || null,
      budgetPotDisplayLabel: plan.budgetPotDisplayLabel || plan.budgetPotLabel || plan.budget_pot_label || null,
...
const CaseWorkspaceContext = createContext({
  ...
  reopenCaseRecovery: () => Promise.resolve({}),
...
export const CaseWorkspaceProvider = ({ caseId, applicationId = null, children }) => {
  ...
  const reopenCaseRecovery = useCallback(async payload => {
    if (!caseId) {
      const error = new Error("Case not loaded.");
      error.status = 400;
      throw error;
    }
    const response = await apiFetch(`/api/cases/${caseId}/reopen-recovery`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload || {}),
    });
    ...
    if (!response.ok) {
      const message =
        ...
        detail?.detail ||
        detail?.error ||
        `Failed to reopen the case plan (${response.status})`;
      const error = new Error(message);
      error.status = response.status;
      error.details = detail;
      throw error;
    ...
    setState(prev => {
      if (!prev.caseData) return prev;
      const reopenedPlan = detail?.actionPlan || null;
      const nextPlans = reopenedPlan
        ? (prev.caseData.actionPlans || []).map(plan => (
            String(plan.id) === String(reopenedPlan.id)
              ? { ...plan, ...reopenedPlan, interventions: plan.interventions || reopenedPlan.interventions || [] }
              : plan
          ))
        : prev.caseData.actionPlans;
      return {
        ...prev,
        caseData: {
          ...prev.caseData,
          status: detail?.caseStatus || "active",
          closedAt: null,
          actionPlans: nextPlans,
        },
      };

...
const InterventionAssessmentWidget = ({ actions, metadata = {}, toggleHelpPanel, workspaceEntry }) => {
  ...
  const [letterWorkflows, setLetterWorkflows] = useState({ approval: null });
  ...
  const approvalSelectionAppliedRef = useRef(null);
  const pendingApprovalStepRef = useRef(null);
  ...
  const hasBlockingReviewStage = useMemo(() => {
  ...
  const statusBadgeColor =
    completionNote && isRejectedDecisionOutcome
      ? "red"
      : completionNote
        ? "green"
        : "blue";
  ...
  useEffect(() => {
    ...
    const loadedInterventionIds = [
      ...
      .filter(Boolean);
    if (requestedInterventionId && !loadedInterventionIds.includes(requestedInterventionId)) {
      return;
    }
  ...
  useEffect(() => {
    ...
    const loadWorkflows = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setLetterWorkflows({ approval: null });
  ...
  useEffect(() => {
    ...
    const handleNew = event => {
      ...
      if (hasBlockingProposal) {
        setError("A proposal is already in progress. Resume it from the table.");
        setSuccessMessage("");
        return;
      }
  ...
  useEffect(() => {
    if (!approvalWorkspaceEntry) return;
    const plans = caseData?.actionPlans || [];
    if (!plans.length) return;
    const approvalKey = `${caseId || "case"}:${approvalWorkspaceEntry.key}`;
    if (approvalSelectionAppliedRef.current === approvalKey) return;
    const reviewTargets = plans
      .flatMap(plan =>
        (Array.isArray(plan?.interventions) ? plan.interventions : []).map(intervention => ({
          ...intervention,
          actionPlanId: intervention?.actionPlanId || plan?.id || null,
        }))
      )
      .filter(item => {
        const statusKey = String(item?.status || "").toLowerCase();
        if (["submitted", "in_review"].includes(statusKey)) return true;
        const requestedStepKey = String(approvalRequestedStep || "").toLowerCase();
        if (requestedStepKey !== "communication") return false;
        return resolveInterventionApprovalLetterFollowUp(item).eligible;
      });
    const requestedInterventionId = approvalWorkspaceEntry.interventionId
      ? String(approvalWorkspaceEntry.interventionId)
      : null;
    const target =
      (requestedInterventionId
        ? reviewTargets.find(item => String(item?.id) === requestedInterventionId)
        : null) ||
      reviewTargets.sort(
        (left, right) =>
    ...
    if (target.actionPlanId && typeof setSelectedActionPlanId === "function") {
      setSelectedActionPlanId(target.actionPlanId);
    }
    setForm(prev => ({
      ...prev,
  ...
  useEffect(() => {
    const plans = caseData?.actionPlans || [];
    if (!plans.length) return;
    if (pendingApprovalStepRef.current && !selectedDraftId) return;
    ...
    const isInReviewValue = value => String(value || "").toLowerCase() === "in_review";
    ...
    const hydrate = draft => {
      ...
      const approvalLetterFollowUp = resolveInterventionApprovalLetterFollowUp(draft);
      const appliedRevisionContext =
        approvalLetterFollowUp.eligible && approvalLetterFollowUp.isRevision
          ? {
              kind: "approved_intervention",
              sourceInterventionId: approvalLetterFollowUp.sourceInterventionId || draft.id || null,
              sourceActionPlanId: approvalLetterFollowUp.sourceActionPlanId || draft.actionPlanId || null,
              sourceStatus: approvalLetterFollowUp.sourceStatus || draft.status || "approved",
              sourceTitle: approvalLetterFollowUp.sourceTitle || draft.title || "this approved intervention",
            }
          : null;
      ...
      setRevisionContext(revision || appliedRevisionContext);
      setCompletionNote(
        approvalLetterFollowUp.eligible
          ? {
              type: "success",
              header: approvalLetterFollowUp.letterSent
  ...
  const buildProposalPayload = useCallback(
    statusValue => {
      ...
      const isDecisionPayload = ["approved", "rejected", "changes_requested"].includes(normalizedStatus);
      ...
      return {
        ...
        metadata: {
          ...
          review: {
            ...
            decision: isDecisionPayload ? (form.decisionOutcome || normalizedStatus) : "",
            decisionNotes: isDecisionPayload ? (form.decisionNotes || "") : "",
  ...
  const handleSave = useCallback(
    async ({ silent = false } = {}) => {
      ...
      const targetStatus = isChangesRequestedStatus
        ? "changes_requested"
        : isReviewStageStatus
          ? statusValue
          : "draft";
      ...
      try {
        const reviewTargetId =
          isReviewStageStatus && activeInterventionIdValue
            ? Number(activeInterventionIdValue)
            : null;
        const existingDraft = findEditableDraft();
        const actionPlanId = Number(form.actionPlanId);
        if (
          Number.isInteger(reviewTargetId) &&
          reviewTargetId > 0 &&
          typeof updateInterventionRecord === "function"
        ) {
          const updated = await updateInterventionRecord(actionPlanId, reviewTargetId, payload);
          setSelectedDraftId(updated?.id || reviewTargetId);
          setHydratedDraftId(updated?.id || reviewTargetId);
        ...
        if (!silent) {
          setSuccessMessage(targetStatus === "draft" ? "Progress saved." : "Changes saved.");
        }
    ...
    async event => {
      ...
      try {
        ...
        const activeTargetId =
          activeInterventionIdValue && Number.isInteger(Number(activeInterventionIdValue)) && Number(activeInterventionIdValue) > 0
            ? Number(activeInterventionIdValue)
            : null;
        const existingDraft = findEditableDraft();
        const actionPlanId = Number(form.actionPlanId);
        const saved = activeTargetId && typeof updateInterventionRecord === "function"
          ? await updateInterventionRecord(actionPlanId, activeTargetId, payload)
          : existingDraft && typeof updateInterventionRecord === "function"
  ...
  const getDecisionBlockingIssues = useCallback(
    ({ requireActiveIntervention = true } = {}) => {
      ...
      if (requireActiveIntervention && !activeInterventionIdValue) {
        reasons.push("Select a proposal in review before submitting a decision.");
        targetStep = targetStep || "decision";
      }
  ...
  const handleSubmitDecision = useCallback(
    async event => {
      ...
      if (!isReviewStageStatus) {
        setError("Only proposals in review can be decided.");
        return { ok: false };
      }
        ...
        if (outcome === "approved") {
          ...
          if (outcome === "rejected") {
            ...
            setCompletionNote({
              ...
              body: isRevisionMode
                ? `The revision was denied and ${revisionSourceTitle} was left unchanged.`
                : "The proposal was denied and closed. Start a new proposal from the Interventions table when needed.",
  ...
  const buildSentLetterCompletionNote = useCallback(() => {
    ...
    return completionNote || {
      ...
      body: isRevisionMode
        ? `The revision was denied and ${revisionSourceTitle} was left unchanged.`
        : "The proposal was denied and closed.",
      showDecisionLetters: false,
    };
  }, [
    completionNote,
  ...
  const handleSendPreparedDecisionLetter = useCallback(async () => {
    ...
    const result = await handleSendDecisionLetter({
      interventionId:
        isRevisionMode && revisionSourceInterventionId
          ? revisionSourceInterventionId
          : activeInterventionIdValue,
  ...
  useEffect(() => {
    if (isApprovedDecisionOutcome) return;
    setApprovalLetterPackGenerated(false);
    setApprovalLetterPackTabId("client");
    setClientLetterBody("");
    setSendingLetterError(null);
  ...
  const planStepContent = (
    <SpaceBetween size="m">
      ...
      {hasBlockingReviewStage && !isReviewStageStatus && (
        <Alert type="warning">
          A proposal is awaiting review. Resolve it before starting a new proposal.
        </Alert>
      )}
        ...
        actions={
          <SpaceBetween direction="horizontal" size="s">
            ...
            {isEditable && !completionNote && (!isReviewStageStatus || !canDecideSubmittedProposal) && (
              <Button
                variant="primary"
                disabled={!isDirty && !isReviewStageStatus}
                onClick={isReviewStageStatus ? handleSubmitProposal : handleSave}
              >
                {isReviewStageStatus
                  ? isRevisionMode
                    ? "Submit change"
      ...
      <div id="intervention-assessment-widget">
        ...
        {completionNote ? (
          <SpaceBetween size="m">
            ...
            {showDecisionLettersPanel ? communicationStepContent : null}
          </SpaceBetween>
        ) : (
          <Wizard

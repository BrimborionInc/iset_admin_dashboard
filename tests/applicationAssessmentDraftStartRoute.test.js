...
function requestJson(server, path, { staffProfileId = 55, body } = {}) {
...
describe('application assessment draft-start caller boundary', () => {
  ...
  const buildResponseRow = () => ({
    ...
    tracking_id: 'ISET-20260812-TEST163',
    ...
    assessment_conflict_declaration_signed_at: '2026-08-01 12:00:00',
    assessment_conflict_declaration_signed_by: 55,
  ...
  beforeAll(async () => {
    ...
    dependencyStore.setAppFactoryTestDependencies({
      ...
      authnMiddlewareFactory: () => (req, _res, next) => {
        const staffProfileId = Number(req.headers['x-test-staff-profile-id'] || 55);
        req.auth = {
          subjectType: 'staff',
  ...
  test('the assigned Regional Manager starts the exact submitted application on first assessment save', async () => {
    const response = await requestJson(server, '/api/cases/187', {
    ...
    expect(statusWrites[0].params).toEqual(['in_review', 'in_review', null, 'none', null, 163]);

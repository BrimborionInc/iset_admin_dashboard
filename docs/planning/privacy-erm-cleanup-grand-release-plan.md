...
- Finance allocation evidence previously had upload/delete/presign routes that accepted raw object keys. DEV now requires finance-role access, tracks new uploads as owned pending finance evidence, and allows presign/delete only for keys referenced by allocation/pot evidence metadata or owned pending uploads.
...
| Finance allocation evidence fetched or deleted by raw object key | High | DEV allocation evidence upload/delete/presign now requires finance-role access, actor-owned pending uploads, and DB metadata provenance before object-store presign/delete |

...
Sprint `4A` completed the synthetic native build-preservation contract from
baseline `ef5ee8c2f687effde7242c167c61baa6919e3f35`. The direct wrapper retains
its native command behavior while a narrow synchronous helper now owns the
exact four generated paths, including portal `publicBuildInfo.js`, and two
isolated output roots. Eleven focused tests prove exact declarations, injected
success/failure restoration, absent-file restoration, sentinels, fail-closed
path admission, restoration-failure evidence, import boundaries and independent
temporary-root absence. No product build, environment input, qualification
...
Sprint `5C` subsequently completed narrowed Phase 5 at clean pushed commit
`9be6881c7ee6bd03caa00d9e7c3de4f3d6f2327e`. The historical raw-DDL change
remains unclassified; prospective evidence retains exact raw and stable
structural proof for all 25 objects. The deliberate-failure, replacement-
interruption and normal rollback paths have exact admission, process evidence
and fresh independent 13-scope zero-residue proof.

### Phase 6 Merged Mapping Boundary

There is no standalone Phase 6 sprint, implementation programme or exit gate.
Only the ownership/dependency mapping needed by an authorized Phase 4, 5, 7 or
8 check is added with that check. This does not authorize selector tooling,
pack migration or a separate mapping corpus. Unknown or ambiguous scope
continues to require the unchanged authoritative full gate.

### Narrowed Phase 7 Breakdown

Phase 7 has two bounded sprints only:

| Sprint | Objective and boundary |
| --- | --- |
| `7A` | **Completed definition only.** From repository source, retained local TEST manifests, docs and local configuration shape, define the exact TEST profile/account/region/operator and remote role, ASG/SSM membership, target groups, current/rollback objects, deployed provenance files, bounded SSM read, IAM actions, negative cases, evidence and one merged impact map. Edit only the architecture and plan; no AWS, TEST, network, database, service, secret or implementation access. |
| `7B` | **Completed implementation and execution.** One task-specific script/test passed 47/47 synthetic cases, the consolidated IAM admission review, and exactly three read-only TEST attempts against the exact retained r31 manifest. All three proved identical identity, source, resources, artifact metadata and deployed provenance, with terminal SSM processes, content-addressed evidence and no residue. `releaseAuthority: none`; the current gate remains authoritative. Stop for Bill's Phase 7 review; Phase 8 is not automatic. |

TEST SES remains disabled to prevent accidental delivery. Phase 7 must not
change its deny policy, enable SES or send email. The complete executable
contract, exact resources, IAM decision boundary, incremental Phase 6 mapping,
commands and copy-ready `7B` authorization are in the
[narrowed Phase 7 design](./release-qualification-harness-target-architecture-2026-08-10.md#narrowed-phase-7-sprint-design).

### Narrowed Phase 8 Breakdown

Bill accepted narrowed Phase 7 and authorized definition-only Sprint `8A`.
Sprint `8A` selects the existing deployed CFA-signing contract as the one
stateful TEST workflow. It is smaller and more controllable than R1 intake for
this boundary: one authenticated applicant, one funding-agreement signing
chain, one product event type and one signed-object family, with no concurrency
journey. Intacct is outside the live PATH solution and is excluded.

Phase 8 has three bounded sprints, including this definition sprint:

| Sprint | Objective and boundary |
| --- | --- |
| `8A` | **Completed definition only.** Select CFA signing; record the exact identity/resource declarations, attempt-owned fixture/effects, persistent assertions, no-email gate, cleanup/recovery and independent residue contract, complete IAM admission matrix, duration, merged impact mapping and later stop rules. Only the two controlling documents changed; no AWS, TEST, database, product HTTP, fixture, IAM/configuration or implementation operation occurred. |
| `8B` | **Completed native contract closure and read-only admission.** The frozen admission proves exact identity, two stable 22-object full-DDL snapshots, the 68-statement catalogue, eight admitted prerequisites, no-email state, bounded evidence transport and remote cleanup. It performed no fixture or product mutation. |
| `8C` | **Conditional stateful execution; live mutation remains unauthorized.** The first authorization stopped before AWS/TEST on local lifecycle defects. Bounded `8C-R1` certified the lifecycle locally. After the embedded r31 DEV authority expired, bounded `8C-P1` separately proved its original checksum/timing and issued three matching 75-minute CFA-only provenance attestations; none renews DEV `GO` or has release authority. Stateful execution still requires separate approval against the frozen `8C-R1` commits, accepted `8B` catalogue and an unexpired accepted attestation. It must re-prove every prerequisite and zero baseline, run one interruption/recovery proof and exactly three clean attempts. Any email/SES attempt, mismatch, cleanup failure or residue stops without repair/rerun. |

The detailed selection, alternatives, exact declared resources, unresolved
live-proof boundary, capability matrix, verification and copy-ready `8B`
authorization are in the [narrowed Phase 8 design](./release-qualification-harness-target-architecture-2026-08-10.md#narrowed-phase-8-sprint-design).
...
- Current authorized phase/sprint: Sprint `8C-P1` is complete. The legacy r31 manifest remains expired for ordinary Phase 7/release use. The bounded explicit mode checksum-validated the unchanged manifest and original DEV artifact, proved that the original DEV `GO` was unexpired when deployment admission began, and then produced exactly three passed CFA-only attestations. All three freshly proved exact TEST operator/remote identity, the unchanged r31 admin/portal/shared tuple, both healthy target groups, four current/rollback artifacts, deployed provenance, source stability, terminal processes and independent no-write residue evidence; their stable semantic SHA-256 is `350cc466...f255945`. Each attestation expires 75 minutes after issuance, records `releaseAuthority: none`, and is structurally prohibited from deployment, TEST qualification, PROD admission or another candidate. This is not renewed DEV `GO`. No build, deployment, SQL/database, product HTTP, Cognito/product-object, SES/email, IAM/configuration or PROD action occurred. Phase 8 remains incomplete; stateful Sprint `8C` requires Bill's separate acceptance and authorization against the frozen `8C-R1` commits, accepted `8B` catalogue and an unexpired accepted attestation.
...
- Next required decision/proposed work: Bill accepts Sprint `8C-P1` and separately authorizes resumed Sprint `8C` only against the frozen admin/portal `8C-R1` commits, accepted `8B` catalogue and an unexpired accepted CFA-only attestation under the existing conditional stateful contract. Expired attestations cannot be extended or relabelled. The current authoritative gate remains unchanged.
- Environment operations authorized: none currently. Sprint `8C-P1` completed only its recorded read-only AWS/SSM provenance effects; stateful TEST, SQL/database, product HTTP, Cognito/product S3, SES/email, deployment, PROD and Phase 9 remain unauthorized.
- Product or harness implementation authorized: none currently. Sprint `8C-P1` has ended; resuming stateful `8C` requires a separate prompt and makes no implementation change.
...
| `5C` prospective correction and Phase 5 | completed - interruption and rollback safety certified | [The completion checkpoint](./release-qualification-harness-target-architecture-2026-08-10.md#sprint-5c-prospective-raw-ddl-correction-and-phase-5-completion) preserves the historical raw-DDL change as unclassified; records exact raw and separately stable structural DDL identities for all 25 objects; narrows the only excluded value to the live-observed numeric InnoDB table-level `AUTO_INCREMENT` counter; and records 41/41 focused tests. A fresh verifier proved the abandoned interruption attempt had 13/13 zero residue. One replacement interruption and one normal rollback-only attempt then passed exact admission, process absence, exact raw before/after comparison, stable structured identity and fresh independent 13/13 zero-residue verification. | Bill accepted Phase 5 at clean pushed `9be6881`; Phase 6 is merged mapping only and has no standalone authorization |
| `7A` | completed - read-only TEST control-plane execution defined | [The narrowed Phase 7 design](./release-qualification-harness-target-architecture-2026-08-10.md#narrowed-phase-7-sprint-design) fixes the exact `nwac-test` operator/account/region, remote role, dynamic ASG/SSM membership, two target groups, r31 manifest, two provenance files, four current/rollback keys, bounded SSM read, declared control-plane effect, evidence, negative cases and exact IAM/configuration stop. It adds only the Phase 7 ownership/impact mapping required by the proposed check. TEST SES remains disabled. No AWS, TEST, network, database, service, secret, IAM/configuration or implementation access occurred. | Bill separately authorizes Sprint `7B` with the exact copy-ready boundary in the architecture; no TEST access, Phase 8, admission or authority change is automatic |
| `7B` | completed - read-only TEST identity and provenance certified | [The completion checkpoint](./release-qualification-harness-target-architecture-2026-08-10.md#sprint-7b---completion-checkpoint) records a complete IAM admission review, 47/47 synthetic cases and three passed content-addressed live attempts. All three shared one normalized semantic hash and proved exact operator/remote role, ASG/SSM/target scope, four artifacts, r31 deployed provenance, source stability, terminal processes and no residue. TEST SES remained disabled; only SSM history and short-lived remote read processes were created. | Bill reviews Phase 7 completion and separately selects/authorizes the exact single stateful TEST workflow for Phase 8; no pack promotion, admission or authority change is automatic |
| `8A` | completed - one stateful TEST workflow defined | [The narrowed Phase 8 design](./release-qualification-harness-target-architecture-2026-08-10.md#narrowed-phase-8-sprint-design) selects deployed CFA signing as the smallest critical no-concurrency workflow; records exact TEST/resource declarations, attempt-owned fixture/effects, persistent assertions, no-email gate, cleanup/recovery and independent residue boundary, consolidated IAM/capability matrix, 15-minute attempt/75-minute sprint limits and the merged impact map. It separates source/config declarations from the exact live DDL, workflow, post-environment principal, notification and residue facts that `8B` must prove read-only. No AWS, TEST, database, SQL, product HTTP, Cognito/S3 fixture, IAM/configuration, implementation or other environment operation occurred. | Bill separately authorizes Sprint `8B` using the exact copy-ready boundary in the architecture; only DDL-admitted prerequisite reads may follow metadata proof, and no TEST mutation, Sprint `8C`, admission, promotion or authority change is automatic |
| `8B` | incomplete - consolidated IAM admission stop | [The governed stop](./release-qualification-harness-target-architecture-2026-08-10.md#sprint-8b---consolidated-iam-admission-stop) records the 22-object/68-statement/19-residue-scope local candidate, 44 passing admin tests, passing portal CFA suites and frozen candidate digests. The consolidated review proved the exact operator and complete local/remote policy sources, but `SES_backend` is effectively allowed SES send while the accepted contract requires an explicit deny. No Phase 7 re-admission, SSM dispatch, database connection, SQL, product HTTP, Cognito/S3 fixture or SES action followed. The retained stateful outer branch also has an unclosed attempt/workflow/fixture-stamp command binding. | The IAM owner applies the recorded explicit deny, then Bill separately authorizes only the bounded Sprint `8B` continuation recorded at the stop; Sprint `8C` remains unauthorized |
| `8B` continuation | incomplete - deterministic evidence-transport stop | [The continuation stop](./release-qualification-harness-target-architecture-2026-08-10.md#sprint-8b-continuation---evidence-transport-stop) records the corrected attempt/workflow/fixture-stamp bindings, 46 passing admin assertions, passing portal suites, complete IAM pass with both SES denies and one passed Phase 7 re-admission. The sole CFA admission's remote preflight exited `0`, but SSM returned exactly 24,000 stdout characters and truncated the evidence envelope; the local parser stopped once. Partial evidence is immutable; cleanup and process termination passed; no fixture/product/database-write/SES effect occurred. | Bill separately decides whether to authorize one bounded evidence-transport correction and a new admission; Sprint `8C` remains unauthorized |
| `8B` evidence transport | completed - read-only CFA admission frozen | [The completion checkpoint](./release-qualification-harness-target-architecture-2026-08-10.md#sprint-8b-evidence-transport-correction-and-completion) records the task-specific attempt-owned evidence file, bounded three-chunk reconstruction, complete digest validation, 53/53 admin assertions and passing portal suites. The sole fresh attempt proved two matching raw/structured full-DDL snapshots for 22 objects, the frozen 68-statement catalogue, eight admitted prerequisite reads, no-email state, terminal cleanup and absence of local/remote transport residue. No fixture, product HTTP, Cognito/S3 product write, database mutation/cleanup SQL or SES action occurred. | Bill accepts Sprint `8B` and separately authorizes only Sprint `8C` under its recorded conditional stateful contract; Phase 8 mutation and all later work remain unauthorized |
| `8C` pre-execution | completed - safe local lifecycle stop | [The `8C-R1` checkpoint](./release-qualification-harness-target-architecture-2026-08-10.md#sprint-8c-safe-pre-execution-stop-and-8c-r1-completion) records the pre-AWS review proving that recovery and residue commands were not dispatched, Cognito/bundle cleanup was premature, Cognito absence was not independently proved, stateful evidence used one monolithic result and no durable post-sign interruption existed. The authorized local product and fixture negatives passed; no live prerequisite or effect began. | Bill authorized only bounded `8C-R1`; no live attempt or later phase was automatic |

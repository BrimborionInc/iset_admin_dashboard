...
| `8B` | **Native contract closure and read-only admission.** Under separate approval, repair only the existing admin CFA wrapper/preflight and portal native runner boundaries, add focused synthetic product/fixture/interruption/cleanup/residue/no-email proof, then perform one consolidated IAM review, one Phase 7 read-only re-admission, exactly two stable metadata-only full-DDL/catalog preflights and, between matching proofs, only the finished prerequisite reads admitted by the first DDL proof. No unguarded statement, database write/transaction/cleanup SQL, Cognito/S3 fixture effect or product HTTP. |
| `8C` | **Conditional stateful execution.** Under separate approval from frozen accepted `8B`, re-prove every prerequisite and zero baseline; run one controlled live post-persistence interruption/recovery proof and exactly three clean CFA attempts, with a fresh independent full residue verifier after each. Local controlled product and harness-fixture failures must already be certified. Any email/SES attempt, mismatch, cleanup failure or residue stops without repair/rerun. |
...
- Current authorized phase/sprint: Sprint `8B` is complete after one bounded evidence-transport correction and one fresh live read-only admission. The task-specific remote-file/chunk contract passed 53/53 admin assertions, both portal CFA test files and every required truncation/corruption/timeout/cancellation/cleanup/no-email case. Attempt `phase8b-admission-transport-41e162d1-010e-4458-bdb6-e70888d96646` independently reconstructed a 36,403-byte envelope in three ordered chunks, validated a 139,413-byte admission payload, proved two identical raw/structured full-DDL snapshots for all 22 objects, admitted the frozen 68-statement catalogue and executed only eight guarded prerequisite reads with no-email true and zero postflight ordinary statements. Cleanup proved the evidence file and remote bundle root absent; no Cognito fixture, product S3 object, product HTTP, database write/transaction/cleanup SQL or SES action occurred. The 22-object/68-statement/19-residue-scope executable catalogue is frozen by retained evidence SHA-256 `64d3f579f760370f4963a25cce98fe799c2302fc3a3f1a0e1748c6b1c6c3414e`. Bill selected `P3-1`; the five Phase 3 packs, source controls, portal deferral and RN02/RN04 remain unchanged. Narrowed Phases 4, 5 and 7 are complete; Phase 6 has no standalone sprint. Phase 8 remains incomplete. Sprint `8C`, TEST mutation, promotion, admission and release-authority changes remain unauthorized pending Bill's separate approval.

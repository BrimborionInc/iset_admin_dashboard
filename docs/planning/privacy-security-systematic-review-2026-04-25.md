...
## 2026-07-10 Revalidation Note

The `GPT56-2026-07` security/privacy lane revalidated the current admin/portal route inventories and the high-risk controls above. The original statement that admin feedback detail is System Administrator-only is superseded: current `canReviewAdminFeedback()` access deliberately includes System Administrator, NWAC Administrator, and Regional Manager for the Bugs and Change Requests workflow. The detail/attachment route still applies that shared review predicate before returning report content or presigned attachment URLs.

`npm run smoke:privacy-routes` now reports four static pattern failures, but source tracing found no corresponding guard removal: the conflict-action scan window ends before the later guarded route, feedback review uses the expanded shared predicate, and notification routes pass route identifiers to the access-matrix helper. The focused notification-access test passes. Treat these as stale tripwires to repair in the `test_blind_spots` lane, not as evidence that the current routes are unguarded.

The live denial harness could not be re-run substantively because no real-token/fixture environment was present; it reported 26 skips. The focused portal Cognito-auth suite passed 11 tests. Current specialist portal findings and the schema-proven PROD aggregate follow-up are recorded in `docs/planning/public-portal-data-exposure-coding-review-2026-05-04.md`; new cross-system findings are indexed in `docs/planning/engineering-audit-register.md`.

## 2026-07-10 R0 Static-Smoke Repair

Authorized local-only remediation tranche `R0` repaired the four stale static checks without changing any route authorization policy. Conflict revoke and resolve are now checked as separate Express registrations; feedback detail asserts the current shared `canReviewAdminFeedback()` predicate; and each template, notification-setting, and sender-setting route asserts its route-matrix guard independently, including the current route argument. The checker now extracts the complete matched Express registration instead of scanning a fixed character window, and focused self-tests prove that an unguarded route cannot borrow a guard from a later sibling and that deliberate removal of each formerly stale guard fails. Current result: all 71 checks pass. Live denial coverage is unchanged and still requires real TEST fixtures with `--require-live`.

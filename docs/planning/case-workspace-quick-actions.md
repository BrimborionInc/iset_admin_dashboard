...
- Add existing action plan (current UI gate: non-archived cases; historical-entry roles only)
- Add existing intervention (current UI gate: non-archived cases; historical-entry roles only)
...
- Assign / reassign visibility and scope:
  ...
  - NWAC Administrator: can assign/reassign to any user except System Administrators.
...
- Backload quick-action current visibility: `Add existing action plan`, `Add existing intervention`, and `Upload existing documents` are shown on non-archived cases for `System Administrator`, `NWAC Administrator`, and `Regional Manager`, including application-backed cases. A historical-record warning is required before launch, and the backend enforces the same role boundary.
- Backload quick-action data meaning: `manual_backload` / `entryMode=existing` creates a real historical case record without starting normal approval, payment-packet, signing, checklist, or applicant-notification side effects. It is not a way to bypass workflow for current work.
...
- Close case visibility: System Administrator, NWAC Administrator, Regional Manager.
- Archive case visibility: System Administrator, NWAC Administrator.
...
- Reopen case visibility: System Administrator, NWAC Administrator (closed cases only).
...
- Reopen case status gating: allowed for `ready_to_close` and `closed` (System Administrator + NWAC Administrator); `archived` allowed only for System Administrator.
...
## Historical Backload Decision Trail

The following bullets record the reasoning that led to the implemented widening. They are not current “future” status:

- Users asked for the flexibility to add old action plans and old interventions to cases that also have applications. Action plans and interventions are case-level records, and the backend backload submit paths key to case/action-plan scope rather than requiring an application-less case. The product/reporting risk remains important: `manual_backload` records are real case records that can affect case history, ILMP validation/export readiness, budget burn, finance reporting, and active-plan conflicts while intentionally skipping approval routing, assessment PDFs, CFA versions, payment packets, checklist progression, and applicant notifications.
- The application-less condition was removed with an explicit historical-entry boundary: labels/modal copy remain historical, `metadata.source = 'manual_backload'` and `metadata.entryMode = 'existing'` remain, and payment-packet/lifecycle controls still apply.

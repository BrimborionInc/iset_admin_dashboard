...
2026-09-03 historical-manual amendment rule: feedback `#199` exposed that the 2026-08-09 exact-application guard also rejected a legitimate older case that was entered manually before an application record existed. This is not a general applicationless exception. PATH accepts the amendment only when it is an exact revision of an applicationless source intervention on the same applicationless Action Plan and case, both source records carry the established manual/backload marker, and the proposal and review workflow also remain applicationless. That verified scope now continues unchanged through submission, both review stages, assessment PDFs, approval-letter follow-up, CFA creation and secure messaging, participant signing, and completion queues. Any mixed, missing, new-proposal, or unexplained application lineage still fails closed. The repair is local; TEST and PROD remain unchanged.

2026-09-03 older approved-intervention rule: feedback `#200` exposed a separate usability and integrity gap on Jodie Stephens's application-owned intervention. The row Actions menu could start a revision, but the opened current-intervention view offered no revision action and treated an older approved compatibility proposal without a two-step workflow row as directly editable. The opened view now exposes the same revision/resume action as the row menu, and an exact approved or rejected proposal is sufficient final-decision evidence to lock approved proposal facts. Delivery-status operations remain separate. The repair is local; no business-data repair is required, and TEST and PROD remain unchanged.

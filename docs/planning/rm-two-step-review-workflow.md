...
Root code causes and local prevention status: `generateAndStoreInterventionAssessmentPdf` passed intervention metadata into `storeAssessmentPdfDocument`, but the store helper did not create the normalized intervention-document link. `syncInterventionProposalCompatibility` also overwrote `submitted_at` from the intervention row's latest `updated_at` on decision sync. Local prevention code has been prepared: generated intervention assessment PDFs now pass `interventionIds` into `storeAssessmentPdfDocument`, and the compatibility upsert preserves existing submitted timestamps through final decisions. This prevention code is not deployed to PROD yet; do not assume future submissions are protected until a PROD app release ships it.

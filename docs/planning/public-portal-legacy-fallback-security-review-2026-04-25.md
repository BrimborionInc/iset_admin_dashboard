...
A later TEST validation with `jack@sillery.co.uk` showed a false applicant-account-required block. Cognito confirmed the account is a confirmed TEST applicant-pool user with no groups, and the TEST portal logs showed `role: 'Applicant'` with `tokenClientMatchesPortal: false`. The first mitigation made `../ISET-intake/auth/cognitoAuth.js` prefer the HttpOnly `iset_access` cookie over any stale `Authorization: Bearer` header for browser portal requests, with a regression test for that precedence. That hotfix was deployed to TEST as public-portal-only release `portal-auth-cookie-precedence-20260425-test`, artifact `s3://nwac-test-artifacts/portal/portal-20260425-100229.zip`; both TEST portal targets were healthy afterward, but Bill's retest still failed.

The confirmed root cause was then found on the TEST hosts: `/opt/nwac/portal/server.js` had the new applicant-account gate, but `/opt/nwac/portal/auth/cognitoAuth.js` was still the old February helper that did not populate the token client ID used by the gate. `deploy-portal-test.ps1` staged `auth/` into the artifact but the remote install command never copied it into `/opt/nwac/portal/auth`. The TEST deploy script was fixed to install `auth/`, and `../ISET-intake/auth/cognitoAuth.js` was also hardened to select the portal access/ID token when duplicate cookies exist. This was committed as `5319327` and deployed to TEST as public-portal-only release `portal-auth-helper-copy-20260425-test`, artifact `s3://nwac-test-artifacts/portal/portal-20260425-105458.zip`; both TEST instances had the updated helper on disk afterward and `nwac-test-portal-tg` reported both targets healthy.

Bill then completed TEST validation for the applicant path: login, dashboard, secure messages, message detail/form links, submitted application detail, document list, and a new applicant-to-staff secure message. The TEST database proved that Jack's new smoke message went to Amanda Curtis's resolved staff local user (`user.id = 104`) and not to the colliding applicant row whose numeric `user.id = 54` matched Amanda's `staff_profiles.id`.

PROD promotion completed on 2026-04-25 as public-portal-only release `portal-auth-helper-copy-20260425-prod`. The deploy used `--skip-schema --skip-data --skip-admin --skip-shared`; no PROD schema, data, admin app, or shared artifact was changed. The PROD ASG refresh `790c0880-9bad-4d1a-991a-ede6e593b114` completed successfully. Post-deploy checks showed the `nwac-prod-portal-tg` target healthy, both `https://nwac-public.awentech.ca/healthz` and `https://iset.nwac.ca/healthz` returning `200 {"status":"ok"}`, the release id present in the deployed bundle, and `/opt/nwac/portal/auth/cognitoAuth.js` containing the expected `preferredClientId` / `getCookieValues` duplicate-cookie auth code.

When testing with a staff/admin account in the public portal, the expected result remains an applicant-account-required message rather than access to applicant data.

After the PROD promotion, `bill@sillery.co.uk` appeared to fail public-portal login with `Login succeeded but session not established`. Investigation showed the password login succeeded and set valid `iset_id` / `iset_access` cookies, but `/api/me` returned Anonymous because the hardened auth layer refused to link the applicant-pool token to a shared `user` row when the same email also existed in active `staff_profiles`. PROD data at the time had `staff_profiles.email = bill@sillery.co.uk` and a separate applicant-pool token for the same email. This is expected security behavior after the breach class: staff-profile emails must not be grandfathered into applicant portal sessions by email fallback. A separate non-staff applicant account should be used for PROD portal smoke tests.

...
- Current secure-message guardrail (2026-04-25): case-thread reads must validate case access before seeding or trusting `message_item` rows, and attachment presign/adoption must validate the message's case context before producing download URLs or creating `iset_document` rows. Do not revive broad `/api/admin/messages` list/create behavior; case secure messaging must flow through `/api/cases/:id/messages`.

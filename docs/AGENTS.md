...
- Current ISET Applications table rule (2026-05-07): `/api/applications` is application-based, not one-row-per-case. The Active view excludes closed/archived application lifecycle rows, while All Applications and Closed can show multiple historical applications for the same client/case. The application-case workspace accepts `?applicationId=` so opening a historical row loads that specific application on the shared case.
...
- Current payments transformation status (updated 2026-05-11): NWAC's email payment workflow has completed DEV automation and TEST rehearsal, but has not been rolled out to PROD or enabled for real Finance email sends. Finance/Sage remains the system of financial record; PATH tracks operations-side request, evidence, email handoff, communications, and follow-up state. Before resuming payment work, read `docs/planning/payments-transformation-plan-2026-05-11.md`, `docs/planning/payments-target-operating-model-2026-05-11.md`, and `docs/testing/payments-workflow-automation.md`; perform a fresh read-only PROD preflight and get explicit approval before any PROD payment-email routing change or real send.
...
- Current deploy checkout rule (2026-05-07 follow-up): the WSL2 Ubuntu checkout under `/home/bill/ISET` is the active source of truth for Codex and TEST deployment work. Run TEST `path:deploy` from `/home/bill/ISET/admin-dashboard`; the orchestrator now builds/packages admin and portal TEST artifacts from the WSL admin, portal, and shared trees and deploys them through WSL AWS CLI + SSM. Do not use stale `/mnt/x` or `X:\ISET` checkout guidance for TEST deploys. PROD app deploy from WSL still requires a separate port/revalidation before the next PROD app rollout; plan/schema/data/smoke helpers remain WSL-safe.
...
- Current TEST public portal deploy target: `../ISET-intake`. The PATH deploy orchestrator sets `PORTAL_ROOT = ../ISET-intake`; TEST deploy now packages the WSL portal tree directly in `scripts/path-deploy.js` and installs the artifact under `/opt/nwac/portal` with a sibling link for `/opt/nwac/ISET-intake`. PROD portal app deploy still needs WSL-native port/revalidation before the next PROD app rollout.
...
- Funding Overview signature requests mirror the CFA secure-message pattern. DEV and TEST workflow `52` (`Funding Overview`, `document_type='financial_overview'`) is a manual secure-message attachment that creates a case-scoped Funding Overview version, renders the current read-only Case Workspace / Application Details financial figures, withdraws unsigned prior Funding Overview sends for that case, and stores the client-signed PDF with `signed` in the name. TEST release `20260512-test-funding-overview-signatures` applied the versioning schema, workflow authoring data, and admin/portal app rollout under all-surface maintenance fallback; follow-up TEST portal release `20260512-test-funding-overview-portal-signing` fixed the applicant-signing side so public-portal signatures mark the Funding Overview version signed and link the signed PDF. Automated TEST smoke used temporary Cognito staff/applicant users plus a synthetic case, proved send/view/sign/version/document behavior, and cleaned up the fixture and S3 objects. See `docs/requirements/SentToApplicant/README.md` and `docs/planning/cfa-versioning-spec.md` before changing this flow.

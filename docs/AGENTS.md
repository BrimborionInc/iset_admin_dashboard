...
- Current case/application guardrail (updated 2026-04-26): do not create `iset_application` rows without client/case scope. The old `POST /api/applications/ingest-from-submission` endpoint is retired with `410`; use `POST /api/cases` or the current intake/manual-intake flows so `iset_application.client_id` and `case_id` are set.
...
- Identity-semantics update from 2026-04-23: `getRequesterIdentity(req).userId` and frontend `currentUser.userId` are not safe generic stand-ins for every ID domain. In current admin behavior, case assignment / regional access checks should use `staff_profiles.id`, application-lock ownership uses the authenticated actor subject string, and shared-table audit ownership uses a resolved local `user.id`. Use backend `getRequesterIdentity(req).staffProfileId` or frontend `currentUser.staffProfileId` when comparing against `assigned_to_user_id` / `staff_profiles.id`; do not reuse lock owner IDs or Cognito subject strings for those comparisons.
- Case assignment identity migration from 2026-04-26: DEV now has `iset_case.assigned_staff_profile_id` with an FK to `staff_profiles(id)` through canonical migration `20260426_0001_add_case_assigned_staff_profile_id.sql`. New assignment writes should populate `assigned_staff_profile_id`; keep the legacy `assigned_to_user_id` dual-write only during the transition and do not introduce new logic that treats it as a shared `user.id`. High-risk case-assignment reads should use explicit staff-profile semantics, preferably `assigned_staff_profile_id` with legacy fallback while the transition is open. High-risk API responses now include `assigned_staff_profile_id` / `assignedStaffProfileId` aliases, and frontend code should resolve assignment through those fields before falling back to legacy `assigned_user_id` / `assigned_to_user_id`.
...
- Secure-message actor-domain migration from 2026-04-26: DEV now has typed sender/recipient actor columns on `messages` through canonical migration `20260426_0002_add_message_actor_domain_columns.sql`, with explicit FKs to `user(id)` and `staff_profiles(id)`. New message writes must populate these columns; the old `sender_id` / `recipient_id` pair remains compatibility mailbox state, not the long-term authority for case-message access.

...
- Current deploy checkout rule (2026-05-07): TEST/PROD app deploys still run from the Windows checkout at `X:\ISET\admin-dashboard`; before deploying work developed in WSL, deliberately sync/pull the intended changes into the Windows checkout and inspect that Windows working tree. `path:deploy` packages the Windows tree, not `/home/bill/ISET`.
...
- Docs base path: active WSL checkout `/home/bill/ISET/admin-dashboard/docs`; older Windows checkout `X:\ISET\admin-dashboard\docs` appears in WSL as `/mnt/x/ISET/admin-dashboard/docs`.

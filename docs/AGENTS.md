...
- Prod DB-affecting deploy runs are supposed to auto-capture an Aurora cluster snapshot restore point for `nwac-prod-db` before schema/data mutation starts, but as of 2026-04-24 the reduced role `nwac-prod-codex-operator` still fails that step because `CreateDBClusterSnapshot` tries to tag the snapshot and the role is missing `rds:AddTagsToResource` on cluster snapshots. Treat DB-affecting prod deploys as blocked on IAM until that is fixed.
- Safe workaround rule from 2026-04-24: only rerun a blocked prod deploy with `--skip-schema --skip-data` if you have explicit proof that no canonical schema change is pending and the source/target allowlisted data payload already matches. For the `intake-release` dataset that proof can be a direct checksum comparison of workflow `21` authoring rows and `iset_runtime_config(scope='publish', k='workflow.schema.intake')` between DEV and PROD.

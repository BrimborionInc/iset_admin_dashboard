...
- Current homepage approvals-items rule: the shared `Approvals` queue drives `Approvals Items`, which combines submitted application assessments and new intervention proposals, shows `Province`, `EI status`, and a compact `Timeline target` badge, and exposes `Open workspace` as the only row action. Approval decisions are completed inside the workspace, and `Open workspace` now passes approval-entry context so the application workspace opens the approval-review layout at `Application Assessment > Approval and decision`, while the case workspace opens a four-widget intervention approval layout (`Case header`, `Proposed new intervention`, `Participant details`, `Supporting documents`) with the selected proposal at `Intervention assessment > Record of decision`. For intervention proposals, `Record of decision` is now the approval commit point; letters are a separate follow-up and are no longer a wizard step in the approver flow.

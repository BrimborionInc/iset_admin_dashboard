...
- Supporting Documents and other shared `View document` actions no longer rely on the browser or Microsoft 365 to render raw Word files. `GET /api/documents/:id/presign-download` now detects `.doc` / `.docx`, generates or reuses a cached internal PDF preview under object storage prefix `WORD_PREVIEW_OBJECT_PREFIX` (default `previews/word`), and returns a presigned URL for that preview instead of the original Office object. The preview artifact is cached in object storage only and must not be inserted as a separate `iset_document` row.

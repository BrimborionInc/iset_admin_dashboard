...
- Resolved bug note (2026-06-14): feedback report `#137` tracked the Shayleen McNabb `ISET-20260410-78062A` manual-backload data-flow issue where Participant Details displayed application fallback values while root Participant Details and action-plan ILMP JSON were blank. The prevention code shipped in PROD release `20260605-prod-ilmp-casework-batch`: Add Existing Action Plan captures and backend-validates required Appendix A reporting facts, and historical action-plan/intervention saves seed blank Participant Details from structured backload ILMP data without overwriting staff-entered values. Guarded PROD repair `sql/ops/prod-repair-shayleen-mcnabb-ilmp-safe-fields-20260605.sql` filled only safe Shayleen fields after restore point `path-prod-shayleen-ilmp-safe-repair-20260605151744`; a later live recheck found action plan `53` had BarrierToEmployment `[7,8]`, EIClaimant `2`, previous NOC `41301` / `2021`, and ESDC participant submission `115` was `ready` as of `2026-06-12 21:00:35`. The report was resolved with `sql/ops/prod-resolve-feedback-137-ilmp-backload-20260614.sql`.

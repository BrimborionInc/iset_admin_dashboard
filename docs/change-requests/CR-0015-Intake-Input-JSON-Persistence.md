...
- _2025-11-14 (docs update)_ : Admin docbase (`docs/features/public-portal-security-features.md`) now documents the shared `input_json_state` and TTL behaviour; portal docbase (`../ISET-intake/docs/portal/intake/intake-form.md`) details how `/api/intake-json` leverages the new table and how drafts hydrate from it.
...
- _2025-11-14_: The current `POST /api/draft` Save & Finish Later path now also clears `input_json_state` after persisting `iset_application_draft` so the DB-backed input JSON never lingers when the user returns to the dashboard.

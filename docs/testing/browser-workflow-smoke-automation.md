...
For feedback `#198`, the same live TEST harness adds a disposable application owned by the real ISET Coordinator role, submits its assessment, has the Regional Manager return it to the submitter, and sends the exact Application Overview withdrawal payload through the deployed `PUT /api/cases/:id` boundary. Acceptance requires one atomic `withdraw_application` review event, no active review owner, a closed/withdrawn exact application, preserved assessment body and generated documents, exact-application withdrawal reporting artifacts, completed reporting interventions, and zero fixture residue. A System Administrator recheck does not substitute for this Coordinator journey.

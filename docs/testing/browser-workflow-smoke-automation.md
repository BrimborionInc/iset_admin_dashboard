...
The assessment-start browser boundary retains a versioned raw artifact before validation. It records the exact sanitized PUT targets, request JSON bytes, response status/body bytes, assigned actor, prerequisite evidence, and native before/after snapshots. The completed artifact is serialized once, SHA-256 bound, and replayed through the same strict validator exercised by the focused local test. Only the two documented declaration/save exchanges are admitted; malformed, extra, contradictory, unreadable, or wrong-scope evidence fails with a field-specific code and retained details. If the browser journey stops early, the partial artifact remains in the attempt evidence rather than being replaced by a summary. This evidence boundary does not change product behavior or weaken any product assertion.

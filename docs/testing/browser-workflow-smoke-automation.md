...
The TEST host runner performs exact-prefix AWS object inventory with synchronous CLI children. Treat a pooled loopback socket surviving that event-loop-blocking interval as unsafe for an effectful document upload: the upload POST must request a fresh one-use HTTP connection and must never retry automatically. Keep the inventory and cleanup calls unchanged. Every bounded-fetch failure must retain a query-free target, method, start/dispatch/failure timestamps, elapsed time, timeout/abort state, and the complete recursively serialized `error.cause` fields; authorization, cookies, secrets and token values remain excluded. The local transport regression uses a server in a separate worker, a short keep-alive timeout and a blocked runner interval so server socket expiry is independent of the runner event loop.

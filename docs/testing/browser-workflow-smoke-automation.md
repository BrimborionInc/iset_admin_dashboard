...
Harness-only acceptance corrections do not require redeploying the product candidate. The local operator transfers the exact current runner as an attempt-owned, SHA-256-bound temporary artifact, the TEST host verifies and executes that copy from its attempt-owned `/tmp` path, and both the remote copy and every S3 version are removed and independently checked during cleanup. The transferred process explicitly resolves its dependencies from `/opt/nwac/admin-dashboard/node_modules`; do not assume a `/tmp` script can find deployed packages through its own module ancestry. `--assessment-start-only` limits live product actions to the conflict-declaration/application-status journey while retaining the existing schema preflight, synthetic fixture ownership, object inventory, cleanup, and zero-residue proof. Focused mode must also create and natively recapture its own attempt-owned unrelated-workflow control before browser execution; it must not rely on workflows that a skipped journey would have created, and the exact control case/application/assessment/workflow must remain in the normal independently counted cleanup scope. The selected synthetic application must carry the product-owned signed application-form EI consent answer; a separately uploaded `ei_consent` document is not an equivalent fixture. Before browser navigation, the runner must prove from the exact assigned staff actor's `start_assessment` checklist that consent is already complete, EI verification is the sole missing required item before upload, and the checklist is complete after the selected and sibling application receive their separate EI evidence. The EI document manifest assertion follows the product-owned canonical metadata value `crf`; `CRF` remains the display/request and assessment-row form.

The assessment-start browser boundary retains a versioned raw artifact before validation. It records the exact sanitized PUT targets, request JSON bytes, response status/body bytes, assigned actor, prerequisite evidence, and native before/after snapshots. The completed artifact is serialized once, SHA-256 bound, and replayed through the same strict validator exercised by the focused local test. The product wizard silently saves changed assessment state before completing a `Next` navigation. The focused journey must therefore begin listening immediately before the first eligibility `Next`, capture the first subsequent case mutation without pre-validating away malformed evidence, and treat that exchange—not a later explicit `Save Progress` action—as the first assessment save. Its dispatch token is compared with the post-declaration, pre-navigation snapshot; its native response and immediate post-save snapshot must both prove the one-version selected-application transition to `in_review`. The journey stops before any second navigation or explicit save. Only the two documented declaration/first-assessment-save exchanges are admitted; malformed, extra, contradictory, unreadable, or wrong-scope evidence fails with a field-specific code and retained details. If the browser journey stops early, the partial artifact remains in the attempt evidence rather than being replaced by a summary. This evidence boundary does not change product behavior or weaken any product assertion.

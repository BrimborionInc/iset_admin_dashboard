...
The sibling `/home/bill/ISET/shared` tree is now a Git repo with private GitHub remote `https://github.com/BrimborionInc/iset_shared.git`. The deployment copy helper excludes `.git` directories from staged artifacts, so source control metadata is not shipped to EC2. Treat shared cleanliness and push status as part of normal deploy preflight, like admin and portal.
...
Release-scope ownership boundary: Codex owns deciding which code/config files belong in a TEST or PROD release. Do not ask Bill implementation-level release-composition questions; inspect diffs, source trees, generated artifacts, and runtime-config boundaries, then choose defensibly. Pause only for true business/data/runtime ambiguity with concrete consequences, such as whether to promote a named DEV-published intake runtime schema that appears experimental.

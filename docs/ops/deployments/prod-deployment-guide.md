...
Last reviewed: 2026-07-08 after the admin-user/EI/notification release and shared local Git repo setup.
...
- `/home/bill/ISET/shared` should be a local Git repo and clean before PROD app deploys. At creation time this local repo did not yet have a GitHub remote, so it improves dirty-state protection but still needs a remote push for off-machine recovery.

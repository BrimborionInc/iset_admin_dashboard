...
- The reduced `nwac-prod` role covers compatibility-artifact deploys, prod SQL/dumps via SSM, ASG refresh, automatic prod restore-point capture, and the ALB `path:maintenance:fallback` flow. As verified on 2026-07-13, it does not currently permit the newer `releases/*` immutable objects. A normal immutable-first run therefore fails before compatibility upload; use the explicit `--compatibility-only` recovery only while the live bootstrap still consumes `*-latest.zip`, or obtain separately reviewed IAM authority. It also does not cover broader infra/admin tasks such as WAF changes, SSM env parameter writes, uploads-bucket CORS changes, or Terraform/ACM changes.

...
Last reviewed: 2026-07-08 after the admin-user/EI/notification release and shared-tree clean-source caveat.
...
- If `/home/bill/ISET/shared` is a plain directory rather than a Git repo in the active workspace, the dirty-source guard packages its current files but cannot prove shared cleanliness. Treat shared edits as explicit release scope and verify deployed source markers or checksums after TEST/PROD rollout.

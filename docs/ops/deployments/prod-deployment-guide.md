...
- The reduced `nwac-prod` role covers normal deploys, prod SQL/dumps via SSM, restore-point snapshots, ASG refresh, and the ALB `path:maintenance:fallback` flow. It does not cover broader infra/admin tasks such as WAF changes, SSM env parameter writes, uploads-bucket CORS changes, or Terraform/ACM changes.

...
- If the sibling `/home/bill/ISET/shared` tree is not a Git repo in the active workspace, the dirty-source guard cannot prove shared cleanliness. Treat shared changes as deliberate release scope, record the marker check you will use, and verify the deployed shared source after TEST/PROD rollout.
...
- In this WSL workspace, `/home/bill/ISET/shared` may be a plain directory rather than a Git repo. In that case the orchestrator packages the current shared files but cannot reject uncommitted shared edits; document the intended shared file changes and verify deployed markers or checksums before calling the release complete.

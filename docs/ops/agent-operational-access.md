...
- PROD deploys also run from `/home/bill/ISET/admin-dashboard`. The WSL-native `path:deploy` PROD app step builds/packages the WSL admin tree, sibling portal tree, and sibling shared tree, normally stages immutable `releases/*` objects, updates fixed latest artifacts, then starts and waits for the PROD ASG refresh. As verified 2026-07-13, the reduced role permits the established component prefixes but not `releases/*`; `--compatibility-only` is the explicit pre-EA-028 recovery and any IAM widening still requires the approval procedure below.
...
- Reduced PROD operator role added 2026-04-20 and widened 2026-04-20 for the repo-driven compatibility deploy/migration path: `nwac-prod-codex-operator` / `nwac-prod` cover artifact uploads in `nwac-prod-artifacts` (`admin/*`, `portal/*`, `shared/*`, `ssm-sql/*`, `db-dumps/*`), PROD SSM SQL/dump execution, ASG refresh, PROD DB restore-point snapshots, and the ALB `path-maintenance-fallback` flow. They do not cover `releases/*` immutable objects and still do not allow direct `secretsmanager:GetSecretValue` on `nwac-prod-db-credentials`.

...
- PROD deploys also run from `/home/bill/ISET/admin-dashboard`. The WSL-native `path:deploy` PROD app step builds/packages the WSL admin tree, sibling portal tree, and sibling shared tree, normally stages immutable `releases/*` objects, updates fixed latest artifacts, then starts and waits for the PROD ASG refresh. On 2026-07-13 Bill attached customer-managed policy `PATHProdImmutableReleaseArtifacts` to `nwac-prod-codex-operator`; a role-backed read-only probe successfully listed the previously inaccessible `releases/` prefix. The policy grants prefix-scoped list plus immutable object put/read/attribute access and deliberately omits delete. `PutObject` remains intentionally unexercised until an explicitly approved PROD release stages its real immutable package; `--compatibility-only` remains an explicit pre-EA-028 recovery, not the normal path.
...
- Reduced PROD operator role added 2026-04-20 and widened for the repo-driven deploy/migration path: `nwac-prod-codex-operator` / `nwac-prod` cover artifact uploads in `nwac-prod-artifacts` (`admin/*`, `portal/*`, `shared/*`, `ssm-sql/*`, `db-dumps/*`), PROD SSM SQL/dump execution, ASG refresh, PROD DB restore-point snapshots, and the ALB `path-maintenance-fallback` flow. Customer-managed policy `PATHProdImmutableReleaseArtifacts`, attached 2026-07-13, additionally covers scoped list and put/read verification for `releases/*` while denying no actions and granting no delete. The role still does not allow direct `secretsmanager:GetSecretValue` on `nwac-prod-db-credentials`.

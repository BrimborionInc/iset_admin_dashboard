...
| `#154` Deleted secure message | Bug | `in_progress` | PROD message `1128` contained; admin product fix deployed to TEST, not PROD | Emilie Marion sent a staff-to-applicant secure message to the wrong participant and local delete did not recall it | Next decision is PROD approval. After PROD deploy, live recheck and update feedback log. |
| `#155` New secure message email notification | Change request | `triaging` | Portal product fix deployed to TEST, not PROD | Emilie Marion requested staff Outlook subject show applicant name rather than applicant email | Next decision is PROD approval. After PROD deploy, send/observe a real applicant-origin secure-message notification and update feedback log. |
...
- Both prepared fixes were deployed to TEST in release `20260705-secure-message-batch`; neither has been deployed to PROD.

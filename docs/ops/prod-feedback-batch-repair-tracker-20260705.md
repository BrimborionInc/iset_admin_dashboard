...
| `#96` New supporting docs for non-school/employment applications | Bug | `in_progress` | Rechecked read-only; still blocked on NWAC/Bill document-type and upload-rule decisions; Bill reaffirmed on 2026-07-05 to leave parked | Bill/NWAC document checklist input | Parked until business input arrives. Do not include in a release or implement by guesswork. |
| `#97` Change "Letters of Reference" | Change request | `in_progress` | Rechecked read-only; still blocked on wording/document-type decision for First Nations/Inuit identity evidence; Bill reaffirmed on 2026-07-05 to leave parked | Bill/NWAC document checklist input | Parked until wording/type decision arrives. Do not include in a release or implement by guesswork. |

...
- For TEST/PROD deploys, start with `docs/AGENTS.md`, then the current `deployments/deployment-quick-guide.md` and `deployments/path-deploy-orchestrator.md`. `deployments/release-qualification-runbook.md` is retired historical evidence and must not be run or treated as an acceptance gate.

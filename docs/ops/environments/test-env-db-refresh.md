...
Last reviewed: 2026-05-29 after TEST sandbox demo reset; current commands checked against `package.json`.
...
- 2026-05-29 TEST sandbox demo reset:
  ...
  - Post-reset smoke: `npm run path:deploy:smoke -- --env test` reported both admin targets healthy on port `5001` and both portal targets healthy on port `5000`.
  ...
  - Post-seed smoke: `npm run path:deploy:smoke -- --env test` reported both admin targets healthy on port `5001` and both portal targets healthy on port `5000`.

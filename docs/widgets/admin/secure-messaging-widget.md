...
- `Applicant replied` is reserved for an applicant-origin reply to the sent item. When staff use Reply only to quote an earlier staff-origin sent item in a new staff message, the quoted item keeps its current applicant-facing `Sent` or `Read by applicant` state.
...
- Staff compose displays the case-derived applicant recipient as read-only. The visible `To` label is not routing authority; backend routing remains case/applicant scoped. Do not add a redundant staff self-attestation checkbox: recipient safety comes from the locked case context and server-side scope validation.
- `Move to Deleted` and `Remove from Deleted` are local mailbox cleanup only. `Withdraw sent message` is the explicit recall-style action for plain staff-to-applicant messages; it redacts the live subject/body, preserves audit records, and is unavailable for messages with linked forms or attachments until a reviewed support repair handles those downstream artifacts.

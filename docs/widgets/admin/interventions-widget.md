...
- Opening a current approved intervention exposes `Revise approved intervention` (or `Resume revision draft`) in the intervention view as well as the row Actions menu. Changes to approved school, program, schedule, or cost facts must use that revision/reapproval path.
- An exact approved or rejected compatibility proposal is final-decision evidence even when the older record predates a two-step `iset_review_workflow` row. Those approved facts are read-only, but delivery lifecycle actions such as activate, suspend, complete, and cancel remain separate and available under their existing rules.

...
- `Withdraw application` and `Reopen application` quick actions are role-neutral within normal Application Workspace file access: assigned ISET Coordinators, scoped Regional Managers, and administrators can use them when the application status is eligible. Withdrawal requires a reason, resolves open escalation rows, and moves any exact pre-final application-assessment review to `withdrawn` with no owner in the same case status-update transaction. Its `withdraw_application` event is distinct from assessment recall and preserves the submitted packet and prior review evidence.
...
- The `Docs Requested` toggle is application-scoped and must save with the freshest selected-application row-version token from the workspace payload. Clearing a stale document-request flag is allowed on terminal/decision-recorded applications; starting a new document request on a terminal application remains blocked.

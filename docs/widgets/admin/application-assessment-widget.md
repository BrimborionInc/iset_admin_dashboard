...
- Submitted application assessments are read-only while the application is pending decision. The submitter sees `Recall submission` instead of `Edit`; recall moves the application back to `in_review`, archives the active system-generated assessment/application-form/financial-overview PDFs from the recalled submission, emits `assessment_recalled`, and allows the assessment to be corrected and resubmitted while no decision has been recorded.

...
- The conflict-of-interest gate should refer to the staff member working the application file, not assume the user is the "Client Case Manager".
- Assessment workflow tables, including proposed interventions, other funders, cost lines, and checklist rows, are workflow-ordered/resizable tables rather than general sortable data grids. Do not add column sorting where staff-entered sequence or checklist order carries meaning without a product decision.
- The funding-forms completion step should use clear staff-facing copy: after required forms are signed and submitted, staff mark the application complete and the client becomes active in Case Management if not already active.
...
- Submitted application assessments are read-only while the application is pending decision. The recorded submitter sees `Recall submission` instead of `Edit`; recall moves the application back to `in_review`, records the review workflow at `withdrawn` as a recalled submission, archives the active system-generated assessment/application-form/financial-overview PDFs from the recalled submission, emits `assessment_recalled`, and allows that submitter to correct and resubmit it to Regional Manager review while no decision has been recorded. Resubmission preserves submitter ownership and clears stale reviewer decisions.
- Regional Managers can edit application-assessment drafts while the application is still `in_review` and no two-step review workflow row exists. Submitted assessments remain read-only for Regional Managers; they must use the review actions after submission.
- Regional Managers, NWAC Administrators, and System Administrators can correct the existing EI status dropdown after submission while the application is not final/locked. The backend blocks the correction once an action plan or intervention exists for the case, because EI status may already affect plan/reporting records. ISET Coordinators remain blocked from changing EI status after it is set.
...
- Recalled assessment PDFs are not active redline baselines. After recall and resubmission, the next redline compares against the last active non-recalled submitted assessment that an approver could actually act on.
...
- Other funding rows now carry a funding status (`Confirmed`, `Pending`, `Denied`, or `Unknown / not confirmed`), optional amount, coverage, and notes. Coverage is required only for confirmed other funding; only confirmed other funders generate other-funder approval letters.
...
- Decision-letter signing artifacts resolve the applicant salutation/full name from a shared server-side resolver using case context, application answers, intake payload, and client identity. Generic UI fallbacks such as `Applicant` or `Client` are ignored when real participant identity is available.
...
- Use `npm run smoke:application-assessment:workflow:browser` after changing workflow branching, selected-application persistence, decision letters, row-version propagation, or funding-document completion. The smoke drives conflict declaration, coordinator submission, NWAC approval, approval-letter send, and funding completion in the real local React route with deterministic API fixtures.
- When refreshing the selected application row version after saves, do not call the parent `onRowVersionUpdate` callback from inside this widget's child state updater. Calling the route parent during the child render/update path triggers React render-phase warnings and is guarded by the workflow browser smoke.

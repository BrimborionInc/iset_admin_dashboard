...
- Notes: deployment runbooks were updated for the post-migration source-tree split. Daily work happens in WSL, but TEST/PROD app deploys still run from and package the Windows checkout at `X:\ISET\...`. Before `path:deploy`, deliberately sync/pull WSL changes into the Windows checkout and inspect the Windows `git status`/diff; do not assume `/home/bill/ISET` edits are what the deploy will package.
- Notes: DEV repeat-application testing found a real model gap: `iset_case_assessment` is still keyed by `case_id`, so a second application on the same case can open and overwrite the prior application's assessment. No code/schema/DB fix has been made yet because unrelated PROD changes are deploying on 2026-05-07. Use `docs/planning/application-assessment-application-scope-migration-plan.md` for the planned DEV-first, TEST-rehearsed, PROD-safe migration to application-scoped assessments.
...
- Topic: moving active admin-dashboard work off `/mnt/x`, keeping TEST/PROD deploys on the Windows checkout, WSL-native local dev startup, re-aligning PATH case-funding semantics to `Approved` / `Committed` / `Actual`, correcting the one prod decimal-shift finance record, and fixing the case-header overall-approved fallback bug
- Keywords: `Avoid Git on Windows drives`, `WSL warning`, `/mnt/x`, `/root/ISET/admin-dashboard`, `/home/bill/ISET/path-dev-wsl.code-workspace`, `/home/bill/ISET/admin-dashboard`, `/home/bill/ISET/ISET-intake`, `/home/bill/ISET/shared`, `/home/bill/ISET/intacct-mock-service`, `dev:all`, `Open the DEV Environment`, `Sage Intacct mock`, `WSL local development`, `X:\\ISET\\admin-dashboard`, `deploy from Windows checkout`, `Approved Committed Actual`, `funds approved`, `case header funding`, `Overall $0 approved`, `ISET-20260408-509365`, `CASE-2026-0000070`, `decimal point`, `divide by 100`, `budget pot recalc`

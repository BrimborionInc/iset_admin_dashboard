...
### Sarah Froese duplicate PATH entry and intervention assessment parity

- Codex task title: `Investigate duplicate PATH entry`
- Topic: PROD investigation and cleanup for Sarah Froese appearing twice in PATH Pending Decision, plus DEV-only intervention proposal assessment PDF parity with the new-application approval flow
- Keywords: `Investigate duplicate PATH entry`, `Sarah Froese`, `Pending Decision`, `duplicate intervention proposal`, `case 40`, `CASE-2026-0000040`, `action plan 5`, `action plan 6`, `intervention 10`, `intervention 11`, `proposal 1`, `proposal 2`, `Amanda Curtis`, `case manager assessment PDF`, `case_assessment`, `case_assessment_approved`, `dual-signed assessment`, `intervention assessment parity`
- When to open: the user asks why Sarah Froese appeared twice in Pending Decision, asks what PROD cleanup was applied to her intervention proposals, asks whether Sarah's one-off assessment PDF was generated, asks what will happen when intervention `11` is approved, asks whether intervention approval should create a dual-signed Case manager assessment, or asks whether a deploy from another thread will include the DEV parity change
- Primary docs:
  - `docs/AGENTS.md`
  - `docs/meta/codex-thread-index.md`
  - `isetadminserver.js`
  ...
  - `tmp/prod-repair-sarah-froese-20260427`
- Status: PROD data cleanup completed on 2026-04-27; DEV code refactor pending deployment
- Notes: Sarah Froese's duplicate Pending Decision entry was caused by two submitted intervention proposals linked to different action plans/records after an earlier bug class had been fixed but her historical data was left inconsistent. PROD case `40` / `CASE-2026-0000040` has active action plan `5` and draft/planned action plan `6`; stale duplicate intervention/proposal `10`/`1` was first cancelled for queue safety, then clean-deleted at Bill's request after preserving before/after TSV evidence, leaving surviving proposal/intervention `2`/`11` intact. A one-off system-generated Case manager assessment PDF was created for the surviving Sarah intervention proposal with Amanda Curtis as signatory and stored as `iset_document.id = 1363`. Related design/code findings from the same thread: Madison can currently approve intervention proposal `11` even though it is over $25,000 because the Shelley Stacey amount threshold is coded for application assessment approvals, not intervention proposal approvals; before approval the intervention widget status label is `Submitted - Awaiting EI status verification` and after approval it displays `Approved`; that submitted wording is arguably application-proposal wording and should be revisited separately. The DEV-only refactor in `isetadminserver.js` now aligns intervention proposal assessment PDFs with the new-application pattern: submitted new/revision intervention proposals generate `case_assessment`, approved intervention decisions generate `case_assessment_approved` with case-manager plus approver signatures, revision submissions can still generate redlines, and application-less cases can store assessment PDFs by case scope. These code changes are uncommitted local working-tree changes and will be included in a TEST/PROD deploy from this same checkout unless removed or deployed from a different checkout.

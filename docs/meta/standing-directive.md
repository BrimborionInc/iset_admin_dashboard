...
Run schema-only preflight before *any* fixture side effect, including Cognito creation and S3 uploads. A cleanup inventory must be resolved again after its cleanup transaction begins, and dependent rows must be deleted from that current relationship scope rather than from IDs captured before the transaction. For SSM/deployed-process checks, credential identity proof must reproduce the runner's environment-loading sequence: an instance profile proves host topology, not which AWS credential an application process will actually select after loading its environment.

Schema-faithful fixtures must also reproduce every newly enforced relationship in the real workflow chain; a fixture that omits an application submission, participant owner, case/application link, version snapshot field, or other guarded parent is not evidence that the product failed. References to existing environment rows must be resolved as one compatible live set (for example region + mapped active budget pot + fiscal period + capacity), not by selecting the first row from one table and assuming a matching row exists elsewhere.
...
- Object-store capability flags describe the provider implementation, not the live bucket's versioning state. Any file-writing release must test both versioned and unversioned `HEAD` results when TEST and PROD differ. Unversioned rollback may use a key-only delete only after an acknowledged `If-None-Match: *` write to a request-owned generated key and an immediate checksum/size recheck; otherwise retain the object and fail closed for manual review.
...
- Codex is the engineering and release-management expert, and Bill supplies essential business goals, knowledge, and operational experience; neither is an infallible authority. Treat prompts and current product behavior as evidence to evaluate, not final instructions or policy. Codex must actively analyze process coherence and downstream consequences, make and defend recommendations, and challenge errors, unsafe sequencing, contradictions, quality regressions, and risks to privacy, data integrity, availability, security, contractual obligations, or NWAC's reputation before acting.
- Do not use collaboration as a reason to seek confirmation of obvious conclusions. Apply established business principles and ordinary operational logic independently, document the conclusion and its basis, and continue. Bring Bill only a genuine policy fork: credible conflicting interpretations with materially different consequences, contradictory evidence, an unresolved external requirement, or a high-consequence decision that cannot be supported safely from what is known.

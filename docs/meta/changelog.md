...
## 2026-05-12
- Testing/Workflow/TEST: Ran an automated TEST Funding Overview signature smoke with temporary Cognito staff/applicant users and a synthetic case. The first pass caught a public-portal signing gap where the signing request became signed but the Funding Overview version stayed `sent`; follow-up portal release `20260512-test-funding-overview-portal-signing` fixed that path, and the rerun passed send/view/sign assertions, version status moved to `signed`, and the linked signed PDF used `Funding Overview v1 (signed)` plus a `signed` file name. Synthetic cases, users, Cognito accounts, DB rows, and S3 objects were cleaned up.
...
- Workflow/Documents/DEV: Added a CFA-style manual secure-message signing workflow for Funding Overview. DEV workflow `52` (`document_type='financial_overview'`) creates case-scoped Funding Overview versions, withdraws unsigned prior sends, renders full clean/redline financial overviews from Case Workspace / Application Details figures, and stores the client-signed PDF with `signed` in the document name.

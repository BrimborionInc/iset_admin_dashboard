...
- DEV constrains application submission/version lineage and CFA case/version/document/participant relationships. CFA agreement documents must resolve to the same case/client as their CFA series. Financial Overview signature versions follow the same case-scoped series pattern through `funding_overview_series`, `funding_overview_version`, and `funding_overview_version_documents`; signed versions are immutable history and unsigned prior sends are withdrawn when a new Financial Overview is sent.
- Local R2 migration `20260710_0002_harden_signing_completion_idempotency.sql` adds durable signing-completion identity/lease fields and a unique nullable `iset_document.signing_request_id` link. It is not yet applied to DEV, TEST, or PROD; schema + shared + portal must release together before the new signing path can run.

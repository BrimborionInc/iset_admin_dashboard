...
| **Interventions** | Generic `intervention_type`, basic amounts | Must capture ILMP codes, NOC, duration, costing split, status gating with DB constraints | Alter table: add ILMP columns; enforce integer cost/duration; add check constraints; create `iset_case_intervention_split`, `iset_case_intervention_commitment` |

...
- Staff-facing UI and guidance call the action **Delete**, never archive. Internally, `DELETE /api/documents/:id` changes an eligible active row to `status='deleted'`, writes/updates its lifecycle row, and records a delete event in one database transaction. The stored object is untouched.
...
- Delete eligibility is dependency-based, not a source allowlist. An active document is ordinarily deletable regardless of whether it was uploaded by staff or an applicant, adopted from a secure message, generated by PATH, or retained from a legacy source. Proven workflow evidence remains protected: PATH-generated signed documents; PATH-generated source documents while their signing request is active; known legacy signing artifacts whose state cannot be established safely; documents linked to any CFA or Financial Overview version; and documents linked to a payment record.
- PATH does not try to identify signatures in arbitrary uploaded scans. A manually uploaded signed-looking PDF remains an ordinary document unless PATH itself generated it through the signing workflow or another protected dependency applies.
- Deleting a secure-message attachment removes its `iset_document` entry from Supporting Documents only. The original message, `message_attachment` record, attachment download, and stored object remain intact. Reopening the message uses a status-agnostic `file_path` lookup and does not insert a replacement row, set the deleted row active, or otherwise reverse the staff deletion.
...
- Checklist history does not block Delete and Delete does not roll a completed checkpoint back. Removing the last matching active document prevents only a current or future checkpoint that still requires it from being passed.
- A delete never reverses a submission, signature, approval, payment, message, or other business event. A document linked through `cfa_version_documents`, `funding_overview_version_documents`, `payment_packet_document`, `payment_packet_line.payment_proof_document_id`, or `payment_followup_event.document_id` must be detached through its owning workflow where that workflow permits it; otherwise it remains protected.
...
- `GET /api/admin/messages/:id/attachments` adopts a previously unindexed attachment into `iset_document` with message, client, case, application, applicant, and uploader context when a `case_id` query param is provided. An existing row is found by `file_path` regardless of lifecycle status, so a deleted Supporting Documents entry is not recreated or reactivated.
- `POST /api/finance/payment-packets/:id/documents` validates `iset_document.client_id` matches the packet.
- `DELETE /api/finance/payment-documents/:id` removes the payment evidence link only while the packet is `draft` or `ready_to_send`. Evidence links are immutable after submission to Finance.
...
- Idempotency: the adoption transaction locks and looks up `iset_document` by unique `file_path` without filtering on status. It inserts only when no row exists and can backfill missing ownership fields on an existing row; it never changes lifecycle status.

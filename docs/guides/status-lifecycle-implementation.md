...
| `dormant` | Plans exist but all are closed/archived. Staff-facing labels display this as **No Active Plan**. | All plans closed/archived while case not yet ready to close. |
...
4. **Denial Letter Sent**: Sending an application assessment denial letter through `POST /api/cases/:id/messages` now completes the denied application on the backend. PATH preserves `decision_outcome = 'denied'`, writes `status = 'completed'` / `lifecycle_status = 'closed'`, and records the `decisionLetterSent.denial` marker under the application-scoped decision-letter context. Do not rely on a frontend follow-up call for this transition.
5. **Manual Overrides**: The Application Overview widget can still POST/PUT changes via `PUT /api/cases/:id`, but DEV now writes canonical case lifecycle plus additive application lifecycle/decision fields rather than relying on one overloaded case status value.
6. **Secure Messaging with forms**: Sending a secure message with attached forms from the Application Workspace while status is `submitted` or `in_review` sets `docs_requested_active` and updates the application status to `docs_requested` so the applicant action is still visible.
7. **Manual doc-request toggle**: The Application Overview widget can set/clear `docs_requested_active` without changing application status, and the secure-message flow auto-clears document requests once all signing requests are complete.

...
- System Administrators can use the Case Header `Reopen closed plan` quick action when circumstances change after closeout. It calls `POST /api/cases/:id/reopen-recovery`, requires a reason, only accepts a closed non-archived plan with no other active plan/open intervention proposal, resets ILMP validation/submission to `needs_review`, reopens the plan as active, and writes an internal note plus `case_reopen_recovery` event. `New intervention` leaves completed interventions untouched; `Amend completed intervention` also reopens the selected completed/cancelled intervention as `in_progress` and clears its outcome/actual closeout fields.
...
- ILMP validation now normalizes current intake/case barrier keys such as `funding`, `location`, `lack-of-job-opportunities`, mixed-separator variants like `lack_of_job-opportunities`, `other`, and health variants through `src/lib/ilmpValidationMappings.js`. If barrier-readiness bugs recur, fix the shared mapper rather than adding one-off aliases to payload or validation code.
- Future-dated active plans with linked planned/draft interventions are intentionally still not ILMP-reportable. The validator should explain that linked interventions exist but are not reportable yet; do not make planned/draft or future work reportable just to clear the readiness queue.
- Intervention NOC validity checks load candidate NOC/version pairs from both snake_case and camelCase action-plan/intervention shapes before querying `noc_code`. Valid stored rows such as `related_noc_version=2021` plus `related_noc=42201` should not be blocked as unknown NOCs.
- Archived action-plan submissions are historical/audit rows and should not be selected as the current case ILMP compliance record. Case-level ILMP revalidation should update current non-archived plan submissions, not revive or reset stale archived-plan blockers.
- Withdrawn applications are still distinct from denials, but they now create the same reporting-only ILMP structure when staff use `Withdraw application`: one closed action plan named `Actions leading to withdrawal` plus completed `Career Research and Exploration` and `Employment Counselling` interventions. The case closes with `closure_reason='withdrawn'`, remains excluded from normal casework queues, and stays editable in the Application Form widget for ILMP reporting corrections.

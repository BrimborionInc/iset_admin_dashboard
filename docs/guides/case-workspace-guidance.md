...
- Withdrawn applications are not denied applications. If the only/open application on a case is withdrawn before casework starts, the case should close with `closure_reason='withdrawn'` so it remains findable in Dormant/All client views, and PATH should not create the denial-reporting `Employment Counselling` / `Career Research and Exploration` interventions.
